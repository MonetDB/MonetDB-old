...
OPTprojectionPrefix(Client cntxt, MalBlkPtr mb, int prefixlength)
{
	...
	for( i = 0; i < limit; i++){
		...
		fprintInstruction(MAL_OPT_PROJECTIONPATH, mb, 0, p, LIST_MAL_ALL);
		...
		if ( match && match == prefixlength - q->retc ){
			...
			fprintInstruction(MAL_OPT_PROJECTIONPATH, mb, 0, r, LIST_MAL_ALL);
			...
			for( ; j < limit; j++) {
				...
				if (match &&  match == prefixlength - r->retc ){
					...
					fprintInstruction(MAL_OPT_PROJECTIONPATH, mb, 0, q, LIST_MAL_ALL);
					if( q->argc == r->argc ){
						clrFunction(q);
