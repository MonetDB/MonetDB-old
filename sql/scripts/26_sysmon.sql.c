...
sql_install_26_sysmon(Client c, char *buf, size_t bufsize)
{
	...
	if (pos >= bufsize)
		throw(SQL, "createdb.26_sysmon", SQLSTATE(42000) "SQL script to install is too large");
	printf("# loading sql script: 26_sysmon.sql\n");
	return SQLstatementIntern(c, &buf, "createdb.26_sysmon", 1, 0, NULL);
}
