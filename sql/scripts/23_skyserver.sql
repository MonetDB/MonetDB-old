...
CREATE SYSTEM FUNCTION MS_STUFF( s1 varchar(32), st int, len int, s3 varchar(32))
RETURNS varchar(32)
BEGIN
...
CREATE SYSTEM FUNCTION MS_TRUNC(num double, prc int)
RETURNS double
external name sql.ms_trunc;

grant execute on function MS_TRUNC to public;

CREATE SYSTEM FUNCTION MS_ROUND(num double, prc int, truncat int)
RETURNS double
BEGIN
	IF (truncat = 0)
...
CREATE SYSTEM FUNCTION MS_STR(num float, prc int, truncat int)
RETURNS string
BEGIN
...
CREATE SYSTEM FUNCTION alpha(pdec double, pradius double)
