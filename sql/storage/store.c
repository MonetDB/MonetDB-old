...
#ifdef STORE_DEBUG
	mnstr_printf(GDKerr, "#destroy trans (%p)\n", t);
#endif

	if (res == gtrans && spares < MAX_SPARES && !t->name) {
#ifdef STORE_DEBUG
		mnstr_printf(GDKerr, "#spared (%d) trans (%p)\n", spares, t);
#endif
		trans_drop_tmp(t);
...
load_column(sql_trans *tr, sql_table *t, oid rid)
{
	...
	c->dcount = 0;
	if (bs_debug)
		mnstr_printf(GDKerr, "#\t\tload column %s\n", c->base.name);
	return c;
}
...
load_table(sql_trans *tr, sql_schema *s, sqlid tid, subrids *nrs)
{
	...
	if (isTable(t)) {
		if (store_funcs.create_del(tr, t) != LOG_OK) {
			if (bs_debug)
				mnstr_printf(GDKerr, "#\tload table %s missing 'deletes'", t->base.name);
			t->persistence = SQL_GLOBAL_TEMP;
		}
	}

	if (bs_debug)
		mnstr_printf(GDKerr, "#\tload table %s\n", t->base.name);
...
load_func(sql_trans *tr, sql_schema *s, sqlid fid, subrids *rs)
{
	...
	if (bs_debug)
		mnstr_printf(GDKerr, "#\tload func %s\n", t->base.name);
...
sql_trans_update_schema(sql_trans *tr, oid rid)
{
	...
	if (bs_debug)
		mnstr_printf(GDKerr, "#update schema %s %d\n", s->base.name, s->base.id);
...
load_schema(sql_trans *tr, sqlid id, oid rid)
{
	...
	if (bs_debug)
		mnstr_printf(GDKerr, "#load schema %s %d\n", s->base.name, s->base.id);
...
sql_trans_update_schemas(sql_trans* tr)
{
	...
	if (bs_debug)
		mnstr_printf(GDKerr, "#update schemas\n");
...
load_trans(sql_trans* tr, sqlid id)
{
	...
	if (bs_debug)
		mnstr_printf(GDKerr, "#load trans\n");
...
bootstrap_create_column(sql_trans *tr, sql_table *t, char *name, char *sqltype, int digits)
{
	...
	if (bs_debug)
		mnstr_printf(GDKerr, "#bootstrap_create_column %s\n", name );
...
bootstrap_create_table(sql_trans *tr, sql_schema *s, char *name)
{
	...
	if (bs_debug)
		mnstr_printf(GDKerr, "#bootstrap_create_table %s\n", name );
...
bootstrap_create_schema(sql_trans *tr, char *name, sqlid auth_id, int owner)
{
	...
	if (bs_debug)
		mnstr_printf(GDKerr, "#bootstrap_create_schema %s %d %d\n", name, auth_id, owner);
...
store_load(void) {
	...
	if (first) {
		...
		if (sql_trans_commit(tr) != SQL_OK) {
			mnstr_printf(GDKerr, "cannot commit initial transaction\n");
		}
	...
	if (logger_funcs.log_needs_update())
		if (store_upgrade_ids(gtrans) != SQL_OK)
			mnstr_printf(GDKerr, "cannot commit upgrade transaction\n");
	return first;
}
...
#ifdef STORE_DEBUG
	mnstr_printf(GDKerr, "#store exit locked\n");
#endif
...
#ifdef STORE_DEBUG
	mnstr_printf(GDKerr, "#store exit unlocked\n");
#endif
	MT_lock_unset(&bs_lock);
}
...
trans_init(sql_trans *t, backend_stack stk, sql_trans *ot)
{
	...
	t->name = NULL;
	if (bs_debug) 
		mnstr_printf(GDKerr, "#trans (%p) init (%d,%d,%d)\n",
...
rollforward_create_table(sql_trans *tr, sql_table *t, int mode)
{
	...
	if (bs_debug) 
		mnstr_printf(GDKerr, "#create table %s\n", t->base.name);
...
rollforward_update_table(sql_trans *tr, sql_table *ft, sql_table *tt, int mode)
{
	...
	if (isTable(ft)) {
		if (p && mode == R_SNAPSHOT) {
			...
			if (bs_debug) 
				mnstr_printf(GDKerr, "#update table %s\n", tt->base.name);
	...
	if (pfs->set) {
		for (m = pfs->set->h; ok == LOG_OK && m && n; ) { 
			...
			if (fb->id == pfb->id) {
				...
				m = m->next;
				if (bs_debug) 
					mnstr_printf(GDKerr, "#reset_cs %s\n", (fb->name)?fb->name:"help");
			} else if (fb->id < pfb->id) {  
				node *t = n->next;

				if (bs_debug) {
					sql_base *b = n->data;
					mnstr_printf(GDKerr, "#reset_cs free %s\n", (b->name)?b->name:"help");
				}
				cs_remove_node(fs, n);
				n = t;
			} else { /* a new id */
				...
				m = m->next;
				if (bs_debug) 
					mnstr_printf(GDKerr, "#reset_cs new %s\n", (r->name)?r->name:"help");
			}
		}
		/* add new bases */
...
#ifdef STORE_DEBUG
	mnstr_printf(GDKerr,"#reset trans %d\n", tr->wtime);
#endif
	tr->wtime = tr->rtime = 0;
	return res;
}
...
#ifdef STORE_DEBUG
			mnstr_printf(GDKerr, "#reuse trans (%p) %d\n", tr, spares);
#endif
		} else {
			tr = trans_dup(stk, (parent) ? parent : gtrans, name);
#ifdef STORE_DEBUG
			mnstr_printf(GDKerr, "#new trans (%p)\n", tr);
#endif
			if(tr)
				transactions++;
		}
...
sql_trans_commit(sql_trans *tr)
{
	...
	/* write phase */
	if (bs_debug)
		mnstr_printf(GDKerr, "#forwarding changes %d,%d %d,%d\n", gtrans->stime, tr->stime, gtrans->wstime, tr->wstime);
	...
	if (bs_debug)
		mnstr_printf(GDKerr, "#done forwarding changes %d,%d\n", gtrans->stime, gtrans->wstime);
...
#ifdef STORE_DEBUG
	mnstr_printf(GDKerr,"#sql trans begin %d\n", snr);
#endif
...
{
#ifdef STORE_DEBUG
	mnstr_printf(GDKerr,"#sql trans end (%d)\n", s->tr->schema_number);
#endif
	s->active = 0;
