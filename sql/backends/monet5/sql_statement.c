...
stmt_uselect(backend *be, stmt *op1, stmt *op2, comp_type cmptype, stmt *sub, int anti)
{
	...
	if (op2->nrcols >= 1) {
		...
		case cmp_gte:
			...
			break;
		default:
			MT_fprintf(stderr, "!SQL Unknown operator");
		}
		...
		if (cmptype == cmp_equal_nil) {
			...
			case cmp_gte:
				...
				break;
			default:
				MT_fprintf(stderr, "!SQL2MAL: error impossible select compare\n");
				if (q)
					freeInstruction(q);
				q = NULL;
			}
...
stmt_join(backend *be, stmt *op1, stmt *op2, int anti, comp_type cmptype)
{
	...
	case cmp_joined:
		...
		break;
	default:
		MT_fprintf(stderr, "!SQL2MAL: error impossible\n");
	}
	if (q) {
...
stmt_trans(backend *be, int type, stmt *chain, stmt *name)
{
	...
	case ddl_trans:
		...
		break;
	default:
		MT_fprintf(stderr, "!SQL transaction unknown type");
	}
...
stmt_catalog(backend *be, int type, stmt *args)
{
	...
	default:
		MT_fprintf(stderr, "!SQL catalog operation unknown\n");
	}
