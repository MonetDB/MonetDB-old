...
str PNperiod(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	if (period < 0)
		throw(MAL,"timetrails.period","The period should be >= 0\n");
	cycleDelay = period;
	return MAL_SUCCEED;
}
...
PNgetheartbeat(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	throw(MAL,"timetrails.getheartbeat","Stream table or query '%s.%s' not found",mod,fcn);
}
...
PNheartbeat(Client cntxt,str mod, str fcn, lng ticks)
{
	...
	if (ticks < 0)
		throw(MAL,"timetrails.heartbeat","The heartbeat should be >= 0\n");
	snprintf(buf,IDLENGTH,"%s_%s",mod,fcn);
	for(i = 0; i < pnettop; i++) {
		if(strcmp(pnet[i].modname,userRef) == 0 && strcmp(pnet[i].fcnname,buf) == 0) {
	...
	if (s == NULL)
		throw(MAL, "timetrails.heartbeat", "Could not find function %s.%s\n",mod,fcn);

	msg = PNregisterInternal(cntxt,s->def,0);
	if( msg){
		GDKfree(msg);
		throw(MAL,"timetrails.heartbeat","Cannot access stream, nor active query\n");
	}
	// try it again
	snprintf(buf,IDLENGTH,"%s_%s",mod,fcn);
	for(i = 0; i < pnettop; i++) {
...
PNstatus( Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci, int newstatus){
	...
	if ( pci->argc == 3){
		...
		if ( i == pnettop){
			...
			throw(SQL,"timetrails.pause","Continuous query %s.%s not found\n", modname, fcnname);
		}
...
PNcycles(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci){
	...
	if( i != pnettop)
		...
		throw(SQL,"timetrails.limit","Continuous query not found\n");
	return MAL_SUCCEED;
}
...
wrapup:
	...
	throw(MAL, "timetrails.queries", MAL_MALLOC_FAIL);
}
...
wrapup:
	...
	if (fcnname)
		BBPunfix(fcnname->batCacheid);
	throw(MAL, "timetrails.inputs", MAL_MALLOC_FAIL);
}
...
wrapup:
	...
	if (fcnname)
		BBPunfix(fcnname->batCacheid);
	throw(MAL, "timetrails.outputs", MAL_MALLOC_FAIL);
}
