...
BSKTregisterInternal(Client cntxt, MalBlkPtr mb, str sch, str tbl)
{
	...
	if (s == NULL)
		throw(SQL, "timetrails.register", "Schema missing\n");

	t = mvc_bind_table(m, s, tbl);
	if (t == NULL)
		throw(SQL, "timetrails.register", "Table missing '%s'\n", tbl);
...
BSKTbind(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	*ret = 0;
		...
		if( bskt > 0){
			if( baskets[bskt].winsize >0){
				...
				if( bn){
					...
					throw(SQL,"timetrails.bind","Can not create view %s.%s.%s[%d]\n",sch,tbl,col,baskets[bskt].winsize );
			} else{
	...
	throw(SQL,"timetrails.bind","Stream table column '%s.%s.%s' not found\n",sch,tbl,col);
}
...
BSKTimportInternal(Client cntxt, int bskt)
{
	...
	for( i=0; i < MAXCOLS && baskets[bskt].cols[i]; i++){
		...
		f=  fopen(buf,"r");
		if( f == NULL)
			throw(MAL,"timetrails.basket","Could not access the column %s file %s\n",cname, buf);
		fclose(f);
	}
	...
	for( i=0; i < MAXCOLS && baskets[bskt].cols[i]; i++){
		...
		f=  fopen(buf,"r");
		if( f == NULL){
			msg= createException(MAL,"timetrails.basket","Could not access the column %s file %s\n",cname, buf);
			break;
		}
		...
		case TYPE_hge:
			...
			if( BATextend(b, (bcnt + fsize) / ATOMsize(b->ttype)) != GDK_SUCCEED){
				(void) fclose(f);
				msg= createException(MAL,"timetrails.basket","Could not extend basket %s\n",baskets[bskt].cols[i]);
				goto recover;
			}
			/* append the binary partition */
			if( fread(Tloc(b, BUNlast(b)),1,fsize, f) != (size_t) fsize){
				(void) fclose(f);
				msg= createException(MAL,"timetrails.basket","Could not read complete basket file %s\n",baskets[bskt].cols[i]);
				goto recover;
			}
		...
		case TYPE_str:
			while (fgets(line, MAXLINE, f) != 0){ //Use getline? http://man7.org/linux/man-pages/man3/getline.3.html
				if ( line[j= (int) strlen(line)-1] != '\n'){
					msg= createException(MAL,"timetrails.basket","string too long\n");
					goto recover;
				} else{
					line[j] = 0;
					if( BUNappend(b, line, TRUE) != GDK_SUCCEED){
						msg= createException(MAL,"timetrails.basket","BUNappend on string value failed\n");
						goto recover;
					}
					bcnt++;
				}
			...
			break;
		default:
			msg= createException(MAL,"timetrails.basket","Import type not yet supported\n");
			goto recover;
		}
		(void) fclose(f);
	}
	...
	for( i=0; i < MAXCOLS && baskets[bskt].cols[i]; i++){
		...
		if( cnt != BATcount(b)){
			msg= createException(MAL,"timetrails.basket","Columns mis-aligned %s\n",baskets[bskt].cols[i]);
			goto recover;
		}
		b->tnil= 0;
		b->tnonil= 0;
...
BSKTimport(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
    ...
    bskt = BSKTlocate(sch,tbl);
	if (bskt == 0)
		throw(SQL, "timetrails.basket", "Could not find the basket %s.%s",sch,tbl);
...
BSKTexportInternal(Client cntxt, int bskt)
{
	...
	for( i=0; i < MAXCOLS && baskets[bskt].cols[i]; i++){
		...
		f=  fopen(buf,"w");
		if( f == NULL)
			throw(MAL,"timetrails.export","Could not access the column %s file %s\n",cname, buf);
		fclose(f);
	}
	...
	for( i=0; i < MAXCOLS && baskets[bskt].cols[i]; i++){
		...
		f=  fopen(buf,"w");
		if( f == NULL){
			msg= createException(MAL,"timetrails.export","Could not access the column %s file %s\n",cname, buf);
			break;
		}
		b = baskets[bskt].bats[i];
		assert( b);
		...
		case TYPE_hge:
			...
			if( fwrite(Tloc(b, BUNlast(b)),1,fsize, f) != (size_t) fsize){
				(void) fclose(f);
				msg= createException(MAL,"timetrails.export","Could not write complete basket file %s\n",baskets[bskt].cols[i]);
				goto recover;
			}
		break;
		case TYPE_str:
			msg= createException(MAL,"timetrails.export","Export type string not yet supported\n");
			break;
		default:
			msg= createException(MAL,"timetrails.export","export type not yet supported\n");
		}
		(void) fclose(f);
	}
...
BSKTtumbleInternal(Client cntxt, str sch, str tbl, int stride)
{
    ...
    bskt = BSKTlocate(sch,tbl);
	if (bskt == 0)
		throw(SQL, "timetrails.tumble", "Could not find the basket %s.%s",sch,tbl);
	...
	for(i=0; i< MAXCOLS && baskets[bskt].cols[i]; i++){
		...
		case TYPE_str:
			...
			case 8: ColumnShift(b,lng); break;
				...
				break;
		default: 
			throw(SQL, "timetrails.tumble", "Could not find the basket column storage %s.%s[%d]",sch,tbl,i);
		}
	...
	if( bn){
		if (binsert)
			...
			if( BUNappend(bn, value, TRUE) != GDK_SUCCEED)
				throw(MAL,"timetrails.append","insertion failed\n");
		BATsettrivprop(bn);
...
wrapup:
	...
	throw(SQL, "timetrails.baskets", MAL_MALLOC_FAIL);
}
