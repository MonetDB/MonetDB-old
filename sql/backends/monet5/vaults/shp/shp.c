...
GDALWConnection * GDALWConnect(char * source) {
	...
	if (conn == NULL) {
		mnstr_printf(GDKerr, "Could not allocate memory\n");
		return NULL;
	}
...
GDALWSimpleFieldDef * GDALWGetSimpleFieldDefinitions(GDALWConnection conn) {
	...
	/*if (conn.layer == NULL || conn.handler == NULL || conn.driver == NULL) {
		mnstr_printf(GDKout, "Could not extract columns, initialize a connection first.\n");
		exit(-1);
	}*/
	...
	while( (feature = OGR_L_GetNextFeature(conn.layer)) != NULL ) {
		for(i = 0; i < OGR_FD_GetFieldCount(featureDefn); i++ ) {
		    ...
		    if( OGR_Fld_GetType(hFieldDefn) == OFTInteger )
				mnstr_printf(GDKout, "%d,", OGR_F_GetFieldAsInteger( feature, i ) );
		    else if( OGR_Fld_GetType(hFieldDefn) == OFTReal )
				mnstr_printf(GDKout, "%.3f,", OGR_F_GetFieldAsDouble( feature, i) );
		    else
				mnstr_printf(GDKout, "%s,", OGR_F_GetFieldAsString( feature, i) );

		}
		geometry = OGR_F_GetGeometryRef(feature);
		OGR_G_ExportToWkt(geometry, &wkt);
		mnstr_printf(GDKout, "%s", wkt);
		mnstr_printf(GDKout, "\n");
		CPLFree(wkt);
