...
str FITSexportTable(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	/*	MT_fprintf(stderr,"Number of columns: %d\n", columns);*/
	...
	MT_fprintf(stderr, "\n\n");
	if (texportboolean > 0)		MT_fprintf(stderr, "%d Boolean\tcolumn(s) exported in %d ms\n", boolcols,   texportboolean);
	if (texportchar > 0)		MT_fprintf(stderr, "%d Char\t\tcolumn(s) exported in %d ms\n",    charcols,   texportchar);
	if (texportstring > 0)		MT_fprintf(stderr, "%d String\tcolumn(s) exported in %d ms\n",  strcols,    texportstring);
	if (texportshort > 0)		MT_fprintf(stderr, "%d Short\t\tcolumn(s) exported in %d ms\n",   shortcols,  texportshort);
	if (texportint > 0)		MT_fprintf(stderr, "%d Integer\tcolumn(s) exported in %d ms\n", intcols,    texportint);
	if (texportlng > 0)		MT_fprintf(stderr, "%d Long\t\tcolumn(s) exported in %d ms\n",    lngcols,   texportlng);
	if (texportfloat > 0)		MT_fprintf(stderr, "%d Float\t\tcolumn(s) exported in %d ms\n",   floatcols,  texportfloat);
	if (texportdouble > 0) 		MT_fprintf(stderr, "%d Double\tcolumn(s) exported in %d ms\n",  doublecols, texportdouble);
	*/
...
/*			char *v = GDKzalloc(wid[j-1]);*/
			...
			for(i = 0; i < bsize ; i++)
				GDKfree(v[i]);
			GDKfree(v);
			MT_fprintf(stderr,"#String column load %d ms, BUNappend %d ms\n", tloadtm, tattachtm);
		}
		else {
		...
		MT_fprintf(stderr,"#Column %s loaded for %d ms\t", cname[j-1], GDKms() - time0);
		if (store_funcs.append_col(m->session->tr, col, tmp, TYPE_bat) != LOG_OK) {
			BBPunfix(tmp->batCacheid);
			msg = createException(MAL, "fits.loadtable", SQLSTATE(HY001) MAL_MALLOC_FAIL);
			break;
		}
		MT_fprintf(stderr,"#Total %d ms\n", GDKms() - time0);
		BBPunfix(tmp->batCacheid);
	}
