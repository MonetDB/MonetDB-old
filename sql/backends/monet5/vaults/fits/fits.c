...
str FITSexportTable(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	DEBUG(SQL_ALL, "Number of columns: %d\n", columns);
	...
	if (texportboolean > 0)		DEBUG(SQL_ALL, "%d Boolean\tcolumn(s) exported in %d ms\n", boolcols, texportboolean);
	if (texportchar > 0)		DEBUG(SQL_ALL, "%d Char\t\tcolumn(s) exported in %d ms\n", charcols, texportchar);
	if (texportstring > 0)		DEBUG(SQL_ALL, "%d String\tcolumn(s) exported in %d ms\n", strcols, texportstring);
	if (texportshort > 0)		DEBUG(SQL_ALL, "%d Short\t\tcolumn(s) exported in %d ms\n", shortcols, texportshort);
	if (texportint > 0)			DEBUG(SQL_ALL, "%d Integer\tcolumn(s) exported in %d ms\n", intcols, texportint);
	if (texportlng > 0)			DEBUG(SQL_ALL, "%d Long\t\tcolumn(s) exported in %d ms\n", lngcols, texportlng);
	if (texportfloat > 0)		DEBUG(SQL_ALL, "%d Float\t\tcolumn(s) exported in %d ms\n", floatcols, texportfloat);
	if (texportdouble > 0)		DEBUG(SQL_ALL, "%d Double\tcolumn(s) exported in %d ms\n", doublecols, texportdouble);
...
str FITSdirpat(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	DEBUG(SQL_ALL, "Fulldir: %s - Size: %lu\n", fulldirectory, globbuf.gl_pathc);
