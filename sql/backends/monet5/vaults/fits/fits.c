...
str FITSexportTable(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	if (texportboolean > 0)		DEBUG(SQL_FITS, "%d Boolean\tcolumn(s) exported in %d ms\n", boolcols, texportboolean);
	if (texportchar > 0)		DEBUG(SQL_FITS, "%d Char\t\tcolumn(s) exported in %d ms\n", charcols, texportchar);
	if (texportstring > 0)		DEBUG(SQL_FITS, "%d String\tcolumn(s) exported in %d ms\n", strcols, texportstring);
	if (texportshort > 0)		DEBUG(SQL_FITS, "%d Short\t\tcolumn(s) exported in %d ms\n", shortcols, texportshort);
	if (texportint > 0)			DEBUG(SQL_FITS, "%d Integer\tcolumn(s) exported in %d ms\n", intcols, texportint);
	if (texportlng > 0)			DEBUG(SQL_FITS, "%d Long\t\tcolumn(s) exported in %d ms\n", lngcols, texportlng);
	if (texportfloat > 0)		DEBUG(SQL_FITS, "%d Float\t\tcolumn(s) exported in %d ms\n", floatcols, texportfloat);
	if (texportdouble > 0)		DEBUG(SQL_FITS, "%d Double\tcolumn(s) exported in %d ms\n", doublecols, texportdouble);
