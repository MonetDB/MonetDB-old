...
WLRgetConfig(void){
	...
	if((path = GDKfilepath(0, 0, "wlr.config", 0)) == NULL){
		MT_fprintf(stderr, "wlr.getConfig:Could not create wlr.config file path\n");
		return -1;
	}
	fd = fopen(path,"r");
	GDKfree(path);
	...
	while( fgets(line, MAXLINE, fd) ){
...
#ifdef _WLR_DEBUG_
		MT_fprintf(stderr,"#WLRgetConfig %s\n", line);
#endif
		if( strncmp("master=", line,7) == 0) {
			len = snprintf(wlr_master, IDLENGTH, "%s", line + 7);
			if (len == -1 || len >= IDLENGTH) {
				MT_fprintf(stderr, "wlr.getConfig:master config value is too large");
				goto bailout;
			} else
			if (len  == 0) {
				MT_fprintf(stderr, "wlr.getConfig:master config path missing");
				goto bailout;
			}
		} else
		...
		if( strncmp("error=", line, 6) == 0) {
			...
			if (len == -1 || len >= FILENAME_MAX) {
				MT_fprintf(stderr, "wlr.getConfig:error config value is too large");
				goto bailout;
			}
			s = strchr(wlr_error, (int) '\n');
			if ( s) *s = 0;
		} else{
				MT_fprintf(stderr, "wlr.getConfig:unknown configuration item '%s'", line);
				goto bailout;
		}
	}
	return 0;
...
WLRputConfig(void){
	...
	if((path = GDKfilepath(0,0,"wlr.config",0)) == NULL){
		MT_fprintf(stderr,"wlr.setMaster:Could not access wlr.config file\n");
		return ;
	}
	fd = open_wastream(path);
	GDKfree(path);
	if( fd == NULL){
		MT_fprintf(stderr,"wlr.setMaster:Could not create wlr.config file\n");
		return;
	}
	...
	close_stream(fd);
#ifdef _WLR_DEBUG_
	MT_fprintf(stderr,"#WLRput: batches %d tag " LLFMT " limit "LLFMT " beat %d timelimit %s\n",
...
WLRprocessBatch(void *arg)
{
	...
	if( c == 0){
		MT_fprintf(stderr, "#Could not create user for WLR process\n");
		return;
	}
	c->promptlength = 0;
	c->listing = 0;
	...
	msg = SQLinitClient(c);
	if( msg != MAL_SUCCEED)
		MT_fprintf(stderr,"#Failed to initialize the client\n");
	msg = getSQLContext(c, mb, &sql, NULL);
	if( msg)
		MT_fprintf(stderr,"#Failed to access the transaction context: %s\n",msg);
	if ((msg = checkSQLContext(c)) != NULL)
		MT_fprintf(stderr,"#Inconsistent SQL context: %s\n",msg);

#ifdef _WLR_DEBUG_
	MT_fprintf(stderr,"#Ready to start the replay against batches state %d wlr "LLFMT"  wlr_limit "LLFMT" wlr %d  wlc %d  taglimit "LLFMT" exit %d\n",
	...
	for( i= wlr_batches; i < wlc_batches && !GDKexiting() && wlr_state != WLR_STOP && wlr_tag < wlr_limit; i++){
		...
		if (len == -1 || len >= FILENAME_MAX) {
			MT_fprintf(stderr,"#wlr.process: filename path is too large\n");
			continue;
		}
		fd= open_rastream(path);
		if( fd == NULL){
			MT_fprintf(stderr,"#wlr.process:'%s' can not be accessed \n",path);
		...
		if (sz > (size_t) 1 << 29) {
			close_stream(fd);
			MT_fprintf(stderr, "#wlr.process File %s too large to process", path);
			continue;
		}
		if((c->fdin = bstream_create(fd, sz == 0 ? (size_t) (2 * 128 * BLOCK) : sz)) == NULL) {
			close_stream(fd);
			MT_fprintf(stderr, "#wlr.process Failed to open stream for file %s", path);
			continue;
		}
		if (bstream_next(c->fdin) < 0){
			MT_fprintf(stderr, "!WARNING: could not read %s\n", path);
			continue;
		}

		c->yycur = 0;
#ifdef _WLR_DEBUG_
		MT_fprintf(stderr,"#REPLAY LOG FILE:%s\n",path);
#endif
		...
		do{
			...
			if( mb->errors){
				...
				cleanup();
#ifdef _WLR_DEBUG_
				MT_fprintf(stderr,"#redo transaction error \n");
#endif
				continue;
			}
			...
			if( getModuleId(q) != wlrRef){
                ...
                MT_fprintf(stderr,"#unexpected instruction ");
			...
			if( getModuleId(q) == wlrRef && getFunctionId(q) == transactionRef){
...
#ifdef _WLR_DEBUG_
				MT_fprintf(stderr,"#do transaction tag "LLFMT" wlr_limit "LLFMT" wlr_tag "LLFMT"\n", tag, wlr_limit, wlr_tag);
#endif
				...
				if(  ( tag > wlr_limit) ||
					...
					cleanup();
#ifdef _WLR_DEBUG_
					MT_fprintf(stderr,"#Found final transaction "LLFMT"("LLFMT")\n", wlr_limit, wlr_tag);
#endif
					break;
				} 
#ifdef _WLR_DEBUG_
				MT_fprintf(stderr,"#run against tlimit %s  wlr_tag "LLFMT"  tag" LLFMT" \n", wlr_timelimit, wlr_tag, tag);
#endif
			}
			...
			if ( getModuleId(q) == wlrRef && getFunctionId(q) ==commitRef ){
				...
				if( mb->errors == 0){
					...
					sql->session->level = 0;
					if(mvc_trans(sql) < 0) {
						MT_fprintf(stderr,"#Allocation failure while starting the transaction \n");
					} else {
#ifdef _WLR_DEBUG_
						MT_fprintf(stderr,"#process a transaction\n");
...
#ifdef _WLR_DEBUG_
		MT_fprintf(stderr,"#wlr.process:processed log file  wlr_tag "LLFMT" wlr_limit "LLFMT" time %s\n", wlr_tag, wlr_limit, wlr_timelimit);
#endif
...
{	Client cntxt = (Client) arg;
...
#ifdef _WLR_DEBUG_
		MT_fprintf(stderr, "#Run the replicator %d %d\n", GDKexiting(),  wlr_state);
#endif
	while( wlr_state != WLR_STOP  && !wlr_error[0]){
...
#ifdef _WLR_DEBUG_
			MT_fprintf(stderr,"#now %s tlimit %s\n",clktxt, wlr_timelimit);
#endif
		...
		if( GDKexiting()){
#ifdef _WLR_DEBUG_
				MT_fprintf(stderr, "#Replicator thread stopped due to GDKexiting()\n");
#endif
			MT_lock_set(&wlr_lock);
			wlr_state = WLR_STOP;
			MT_lock_unset(&wlr_lock);
...
#ifdef _WLR_DEBUG_
	MT_fprintf(stderr, "#Replicator thread is stopped \n");
#endif
}
...
{	str timelimit  = wlr_timelimit;
	...
	if( getArgType(mb, pci, 1) == TYPE_timestamp){
		...
		MT_fprintf(stderr,"#time limit %s\n",timelimit);
	} else
	...
	strftime(clktxt, sizeof(clktxt), "%Y-%m-%dT%H:%M:%S.000",&ctm);
#ifdef _WLR_DEBUG_
	MT_fprintf(stderr, "#replicate: wait until wlr_limit = "LLFMT" (tag "LLFMT") time %s (%s)\n", wlr_limit, wlr_tag, (wlr_timelimit[0]? wlr_timelimit:""), clktxt);
#endif
...
#ifdef _WLR_DEBUG_
	MT_fprintf(stderr, "#replicate wait state %d wlr_limit "LLFMT" (wlr_tag "LLFMT") wlc_tag "LLFMT" wlr_batches %d\n",
...
#ifdef _WLR_DEBUG_
	MT_fprintf(stderr, "#replicate finished "LLFMT" (tag "LLFMT")\n", wlr_limit, wlr_tag);
#endif
	return msg;
}
...
#ifdef _WLR_DEBUG_
	MT_fprintf(stderr,"#WLR scheduler forked\n");
#else
	(void) cntxt;
#endif
...
#ifdef _WLR_DEBUG_
		MT_fprintf(stderr,"#WLR replicator initializing\n");
#endif
		MT_sleep_ms( 50);
	}
	return MAL_SUCCEED;
...
#ifdef _WLR_DEBUG_
	MT_fprintf(stderr,"#WLR stop replication\n");
#endif
    MT_lock_set(&wlr_lock);
	if( wlr_state == WLR_RUN)
			wlr_state =  WLR_STOP;
...
#define WLRvalue(TPE)                                                   \
	{	TPE val = *getArgReference_##TPE(stk,pci,5);            \
			if (BUNappend(upd, (void*) &val, false) != GDK_SUCCEED) { \
				MT_fprintf(stderr, "WLRupdate:BUNappend failed"); \
				goto cleanup;                                   \
			}                                                       \
	}
...
WLRupdate(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	case TYPE_str:
		...
		break;
	default:
		MT_fprintf(stderr, "Missing type in WLRupdate");
	}
