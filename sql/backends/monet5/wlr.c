...
WLRgetConfig(void){
	...
	if((path = GDKfilepath(0, 0, "wlr.config", 0)) == NULL){
		Trace(SQL_WLR, M_ERROR, "Could not create wlr.config file path\n");
		return -1;
	}
	fd = fopen(path,"r");
	GDKfree(path);
	...
	while( fgets(line, MAXLINE, fd) ){
		...
		Trace(SQL_WLR, M_DEBUG, "%s\n", line);
		if( strncmp("master=", line,7) == 0) {
			len = snprintf(wlr_master, IDLENGTH, "%s", line + 7);
			if (len == -1 || len >= IDLENGTH) {
				Trace(SQL_WLR, M_ERROR, "Master config value is too large\n");
				goto bailout;
			} else
			if (len  == 0) {
				Trace(SQL_WLR, M_ERROR, "Master config path is missing\n");
				goto bailout;
			}
		} else
		...
		if( strncmp("error=", line, 6) == 0) {
			...
			if (len == -1 || len >= FILENAME_MAX) {
				Trace(SQL_WLR, M_ERROR, "Config value is too large\n");
				goto bailout;
			}
			s = strchr(wlr_error, (int) '\n');
			if ( s) *s = 0;
		} else{
				Trace(SQL_WLR, M_ERROR, "Unknown configuration item '%s'\n", line);
				goto bailout;
		}
	}
	return 0;
...
WLRputConfig(void){
	...
	if((path = GDKfilepath(0,0,"wlr.config",0)) == NULL){
		Trace(SQL_WLR, M_ERROR, "Could not access wlr.config file\n");
		return ;
	}
	fd = open_wastream(path);
	GDKfree(path);
	if( fd == NULL){
		Trace(SQL_WLR, M_ERROR, "Could not create wlr.config file\n");
		return;
	}
	...
	Trace(SQL_WLR, M_DEBUG, "Batches %d tag " LLFMT " limit "LLFMT " beat %d timelimit %s\n",
...
WLRprocessBatch(void *arg)
{
	...
	if( c == 0){
		Trace(SQL_WLR, M_ERROR, "Could not create user for WLR process\n"); 
		return;
	}
	c->promptlength = 0;
	c->listing = 0;
	...
	msg = SQLinitClient(c);
	if( msg != MAL_SUCCEED)
		Trace(SQL_WLR, M_ERROR, "Failed to initialize the client\n");
	msg = getSQLContext(c, mb, &sql, NULL);
	if( msg)
		Trace(SQL_WLR, M_ERROR, "Failed to access the transaction context: %s\n", msg);
	if ((msg = checkSQLContext(c)) != NULL)
		Trace(SQL_WLR, M_ERROR, "Inconsistent SQL context: %s\n", msg);

	Trace(SQL_WLR, M_DEBUG, "#Ready to start the replay against batches state %d wlr "LLFMT"  wlr_limit "LLFMT" wlr %d  wlc %d  taglimit "LLFMT" exit %d\n",
	...
	for( i= wlr_batches; i < wlc_batches && !GDKexiting() && wlr_state != WLR_STOP && wlr_tag < wlr_limit; i++){
		...
		if (len == -1 || len >= FILENAME_MAX) {
			Trace(SQL_WLR, M_ERROR, "Filename path is too large\n");
			continue;
		}
		fd= open_rastream(path);
		if( fd == NULL){
			Trace(SQL_WLR, M_ERROR, "Cannot access path '%s'\n", path);
		...
		if (sz > (size_t) 1 << 29) {
			close_stream(fd);
			Trace(SQL_WLR, M_ERROR, "File %s is too large to process\n", path);
			continue;
		}
		if((c->fdin = bstream_create(fd, sz == 0 ? (size_t) (2 * 128 * BLOCK) : sz)) == NULL) {
			close_stream(fd);
			Trace(SQL_WLR, M_ERROR, "Failed to open stream for file %s\n", path);
			continue;
		}
		if (bstream_next(c->fdin) < 0){
			Trace(SQL_WLR, M_ERROR, "Could not read %s\n", path);
			continue;
		}

		c->yycur = 0;
		Trace(SQL_WLR, M_DEBUG, "Replay log file: %s\n", path);
		...
		do{
			...
			if( mb->errors){
				...
				Trace(SQL_WLR, M_INFO, "%s\n", line);
				fprintFunction(stderr, mb, 0, LIST_MAL_DEBUG );
				cleanup();
				Trace(SQL_WLR, M_DEBUG, "Redo transaction error\n");
				continue;
			}
			...
			if( getModuleId(q) != wlrRef){
				...
				Trace(SQL_WLR, M_DEBUG, "Unexpected instruction");
			...
			if( getModuleId(q) == wlrRef && getFunctionId(q) == transactionRef){
				...
				Trace(SQL_WLR, M_DEBUG, "Do transaction tag "LLFMT" wlr_limit "LLFMT" wlr_tag "LLFMT"\n", tag, wlr_limit, wlr_tag);
				...
				if(  ( tag > wlr_limit) ||
					...
					cleanup();
					Trace(SQL_WLR, M_DEBUG, "Found final transaction "LLFMT"("LLFMT")\n", wlr_limit, wlr_tag);
					break;
				} 

				Trace(SQL_WLR, M_DEBUG, "Run against tlimit %s  wlr_tag "LLFMT" tag" LLFMT" \n", wlr_timelimit, wlr_tag, tag);
			}
			...
			if ( getModuleId(q) == wlrRef && getFunctionId(q) ==commitRef ){
				...
				if( mb->errors == 0){
					...
					sql->session->level = 0;
					if(mvc_trans(sql) < 0) {
						Trace(SQL_WLR, M_CRITICAL, "Allocation failure while starting the transaction\n");
					} else {
						
						/* CHECK */
						// This needs to be fixed along with other functions that use stderr
						Trace(SQL_WLR, M_DEBUG, "Process a transaction\n");
		...
		Trace(SQL_WLR, M_DEBUG, "Processed log file wlr_tag "LLFMT" wlr_limit "LLFMT" time %s\n", wlr_tag, wlr_limit, wlr_timelimit);
...
{	Client cntxt = (Client) arg;
	...
	Trace(SQL_WLR, M_DEBUG, "Run the replicator %d %d\n", GDKexiting(), wlr_state);
	...
	while( wlr_state != WLR_STOP  && !wlr_error[0]){
		...
		if( wlr_timelimit[0]){
			...
			Trace(SQL_WLR, M_DEBUG, "Now %s tlimit %s\n", clktxt, wlr_timelimit);
		 ...
		 */
		if( GDKexiting()){
			Trace(SQL_WLR, M_DEBUG, "Thread stopped due to GDKexiting()\n");
			MT_lock_set(&wlr_lock);
			wlr_state = WLR_STOP;
			MT_lock_unset(&wlr_lock);
			break;
	...
	Trace(SQL_WLR, M_DEBUG, "Replicator thread has stopped\n");
}
...
{	str timelimit  = wlr_timelimit;
	...
	if( getArgType(mb, pci, 1) == TYPE_timestamp){
		...
		Trace(SQL_WLR, M_INFO, "Time limit %s\n", timelimit);
	} else
	...
	Trace(SQL_WLR, M_DEBUG, "Wait until wlr_limit = "LLFMT" (tag "LLFMT") time %s (%s)\n", 
	...
	while ( (wlr_tag < wlr_limit )  || (wlr_timelimit[0]  && strncmp(clktxt, wlr_timelimit, sizeof(wlr_timelimit)) > 0)  ) {
		...
		Trace(SQL_WLR, M_DEBUG, "Wait state %d wlr_limit "LLFMT" (wlr_tag "LLFMT") wlc_tag "LLFMT" wlr_batches %d\n", 
	...
	Trace(SQL_WLR, M_DEBUG, "Finished "LLFMT" (tag "LLFMT")\n", wlr_limit, wlr_tag);
...
WLRstart(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	Trace(SQL_WLR, M_DEBUG, "Forked WLR scheduler\n");
	(void) cntxt;

	// Wait until the replicator is properly initialized
	while( wlr_state != WLR_RUN && wlr_error[0] == 0){
		Trace(SQL_WLR, M_DEBUG, "Initializing WLR replicator\n");
		MT_sleep_ms( 50);
	}
	return MAL_SUCCEED;
}
...
WLRstop(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	Trace(SQL_WLR, M_DEBUG, "Stop replication\n");
    MT_lock_set(&wlr_lock);
	if( wlr_state == WLR_RUN)
			wlr_state =  WLR_STOP;
    MT_lock_unset(&wlr_lock);
...
#define WLRvalue(TPE)                                                   \
	{	TPE val = *getArgReference_##TPE(stk,pci,5);            		\
			if (BUNappend(upd, (void*) &val, false) != GDK_SUCCEED) { 	\
				Trace(SQL_WLR, M_ERROR, "BUNappend failed\n");			\
				goto cleanup;                                   		\
		}                                                       		\
	}
...
WLRupdate(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	case TYPE_str:
		...
		break;
	default:
		Trace(SQL_WLR, M_ERROR, "Missing type in WLRupdate\n");
	}
