...
SQLoptimizeQuery(Client c, MalBlkPtr mb)
{
	...
	if (mb->stop > 0 &&
	    ...
	    getVarConstant(mb,mb->stmt[mb->stop-1]->argv[0]).vtype == TYPE_str &&
	    getVarConstant(mb,mb->stmt[mb->stop-1]->argv[0]).val.sval &&
	    strncmp(getVarConstant(mb,mb->stmt[mb->stop-1]->argv[0]).val.sval, "total", 5) == 0)
