...
rel_xmlelement(mvc *sql, sql_rel **rel, symbol *sym, int f, exp_kind knd) 
{
	...
	if (ns_attrs_elms) {
		...
		if (content) {
			...
			for (n=cl->h; n; n = n->next) {
				...
				if (res) {
					res = rel_binop_(sql, res, c_st, NULL, "concat", 0, card_value);
				} else {
					res = c_st;
				}
			}
	...
	if (!ns_st || !attr_st || !res) 
		return NULL;
	return rel_nop_(sql, exp_atom_clob(sql->sa, tag), ns_st, attr_st, res, NULL, "element", 0, card_value);
}
...
rel_xmlforest(mvc *sql, sql_rel **rel, symbol *sym, int f, exp_kind knd) 
{
	...
	if (elms) {
		...
		for (e = elms->h; e; e = e->next) {
			...
			c_st = rel_nop_(sql, exp_atom_clob(sql->sa, tag), ns_st, attr_st, c_st, NULL, "element", 0, card_value);
			/* lets glue the xml content together */
			if (res) {
				res = rel_binop_(sql, res, c_st, NULL, "concat", 0, card_value);
			} else {
...
rel_xmlcomment(mvc *sql, sql_rel **rel, symbol *sym, int f, exp_kind knd) 
{
	...
	if (!comment_st)
		return NULL;
	return rel_unop_(sql, comment_st, NULL, "comment", 0, card_value);
}
...
rel_xmlattribute(mvc *sql, sql_rel **rel, symbol *sym, int f, exp_kind knd) 
{
	...
	return rel_binop_(sql, attr_name_st, attr_st, NULL, "attribute", 0, card_value);
}
...
rel_xmlconcat(mvc *sql, sql_rel **rel, symbol *sym, int f, exp_kind knd) 
{
	...
	for (; en; en = en->next) {
		...
		if (!concat_st) 
			return NULL;
		if (res) 
			res = rel_binop_(sql, res, concat_st, NULL, "concat", 0, card_value);
		else
			res = concat_st;
	}	
	return res;
...
rel_xmldocument(mvc *sql, sql_rel **rel, symbol *sym, int f, exp_kind knd) 
{
	...
	if (!val_st)
		return NULL;
	return rel_unop_(sql, val_st, NULL, "document", 0, card_value);
}
...
rel_xmlpi(mvc *sql, sql_rel **rel, symbol *sym, int f, exp_kind knd) 
{
	...
	if (!val_st) 
		return NULL;
	return rel_binop_(sql, target_st, val_st, NULL, "pi", 0, card_value);
}
