...
#endif
		default:
			mnstr_printf(GDKerr, "atom_int %d\n", a->data.vtype);
			assert(0);
		}
		a->d = (dbl) val;
		a->data.len = 0;
		if (atom_debug)
			mnstr_printf(GDKerr, "atom_int(%s,%.40g)\n", tpe->type->sqlname, (dbl)val);
		return a;
	}
}
...
atom_string(sql_allocator *sa, sql_subtype *tpe, const char *val)
{
	...
	if (atom_debug)
		mnstr_printf(GDKerr, "atom_string(%s,%s)\n", tpe->type->sqlname, val);
	return a;
}
...
atom_float(sql_allocator *sa, sql_subtype *tpe, double val)
{
	...
	a->data.len = 0;
	if (atom_debug)
		mnstr_printf(GDKerr, "atom_float(%s,%f)\n", tpe->type->sqlname, val);
	return a;
}
...
atom_general(sql_allocator *sa, sql_subtype *tpe, const char *val)
{
	...
	if (atom_debug)
		mnstr_printf(GDKerr, "atom_general(%s,%s)\n", tpe->type->sqlname, val);
