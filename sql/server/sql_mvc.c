...
#define MVC_INIT_DROP_TABLE(SQLID, TNAME)                      \
	...
	if((output = mvc_drop_table(m, s, t, 0)) != MAL_SUCCEED) { \
		mvc_destroy(m);                                        \
		mnstr_printf(GDKerr, "!mvc_init: %s\n", output);            \
		GDKfree(output);                                       \
		return -1;                                             \
	}
...
mvc_init(int debug, store_type store, int ro, int su, backend_stack stk)
{
	...
	mvc_debug = debug&4;
	if (mvc_debug) {
		mnstr_printf(GDKerr, "#mvc_init\n");
	}
	keyword_init();
	if(scanner_init_keywords() != 0) {
		mnstr_printf(GDKerr, "!mvc_init: malloc failure\n");
		return -1;
	}

	if ((first = store_init(debug, store, ro, su, stk)) < 0) {
		mnstr_printf(GDKerr, "!mvc_init: unable to create system tables\n");
		return -1;
	}
	...
	if (first || catalog_version) {
		if(mvc_trans(m) < 0) {
			mvc_destroy(m);
			mnstr_printf(GDKerr, "!mvc_init: failed to start transaction\n");
			return -1;
		}
		...
		if ((msg = mvc_commit(m, 0, NULL, false)) != MAL_SUCCEED) {
			mnstr_printf(GDKerr, "!mvc_init: unable to commit system tables: %s\n", (msg + 6));
			freeException(msg);
			return -1;
		}
	}
			...
			for (node *nn = ss->tables.set->h; nn; nn = nn->next) {
				...
				if(isPartitionedByColumnTable(tt) || isPartitionedByExpressionTable(tt)) {
					...
					if((err = initialize_sql_parts(m, tt)) != NULL) {
						mnstr_printf(GDKerr, "!mvc_init: unable to start partitioned table: %s.%s: %s\n",
...
mvc_commit(mvc *m, int chain, const char *name, bool enabling_auto_commit)
{
	...
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_commit %s\n", (name) ? name : "");
	...
	if (name && name[0] != '\0') {
		...
		if (mvc_debug)
			mnstr_printf(GDKerr, "#mvc_savepoint\n");
		store_lock();
...
build up the hash (not copied in the trans dup)) */
		...
		if (mvc_debug)
			mnstr_printf(GDKerr, "#mvc_commit %s done\n", name);
		return msg;
	}
...
mvc_rollback(mvc *m, int chain, const char *name, bool disabling_auto_commit)
{
	...
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_rollback %s\n", (name) ? name : "");
	...
	m->type = Q_TRANS;
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_rollback %s done\n", (name) ? name : "");
	return msg;
}
...
mvc_release(mvc *m, const char *name)
{
	...
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_release %s\n", (name) ? name : "");
...
mvc_create(int clientid, backend_stack stk, int debug, bstream *rs, stream *ws)
{
	...
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_create\n");
...
mvc_destroy(mvc *m)
{
	...
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_destroy\n");
	tr = m->session->tr;
	if (tr) {
		store_lock();
		if (m->session->active)
...
mvc_bind_type(mvc *sql, const char *name)
{
	...
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_bind_type %s\n", name);
	return t;
}
...
schema_bind_type(mvc *sql, sql_schema *s, const char *name)
{
	...
	if (!t)
		return NULL;
	if (mvc_debug)
		mnstr_printf(GDKerr, "#schema_bind_type %s\n", name);
	return t;
}
...
mvc_bind_func(mvc *sql, const char *name)
{
	...
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_bind_func %s\n", name);
	return t;
}
...
schema_bind_func(mvc *sql, sql_schema * s, const char *name, int type)
{
	...
	if (!func_list)
		return NULL;
	if (mvc_debug)
		mnstr_printf(GDKerr, "#schema_bind_func %s\n", name);
	return func_list;
}
...
mvc_bind_schema(mvc *m, const char *sname)
{
	...
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_bind_schema %s\n", sname);
	return s;
}
...
mvc_bind_table(mvc *m, sql_schema *s, const char *tname)
{
	...
	if (!t)
		return NULL;
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_bind_table %s.%s\n", s ? s->base.name : "<noschema>", tname);
...
mvc_bind_column(mvc *m, sql_table *t, const char *cname)
{
	...
	if (!c)
		return NULL;
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_bind_column %s.%s\n", t->base.name, cname);
	return c;
}
...
mvc_first_column(mvc *m, sql_table *t)
{
	...
	if (!c)
		return NULL;
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_first_column %s.%s\n", t->base.name, c->base.name);
...
mvc_bind_key(mvc *m, sql_schema *s, const char *kname)
{
	...
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_bind_key %s.%s\n", s->base.name, kname);
...
mvc_bind_idx(mvc *m, sql_schema *s, const char *iname)
{
	...
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_bind_idx %s.%s\n", s->base.name, iname);
...
mvc_bind_trigger(mvc *m, sql_schema *s, const char *tname)
{
	...
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_bind_trigger %s.%s\n", s->base.name, tname);
...
mvc_create_type(mvc *sql, sql_schema * s, const char *name, int digits, int scale, int radix, const char *impl)
{
	...
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_create_type %s\n", name);
...
{
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_drop_type %s %s\n", s->base.name, t->base.name);
...
mvc_create_func(mvc *sql, sql_allocator *sa, sql_schema * s, const char *name, list *args, list *res, int type, int lang, const char *mod, const char *impl, const char *query, bit varres, bit vararg, bit system)
{
	...
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_create_func %s\n", name);
	if (sa) {
...
{
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_drop_func %s %s\n", s->base.name, f->base.name);
...
mvc_create_schema(mvc *m, const char *name, sqlid auth_id, sqlid owner)
{
	...
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_create_schema %s %d %d\n", name, auth_id, owner);
...
{
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_drop_schema %s\n", s->base.name);
...
{
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_create_ukey %s %u\n", t->base.name, kt);
...
{
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_create_fkey %s %u %p\n", t->base.name, kt, rkey);
	if (t->persistence == SQL_DECLARED_TABLE)
...
{
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_drop_key %s %s\n", s->base.name, k->base.name);
	if (k->t->persistence == SQL_DECLARED_TABLE) {
...
mvc_create_idx(mvc *m, sql_table *t, const char *name, idx_type it)
{
	...
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_create_idx %s %u\n", t->base.name, it);
...
{
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_drop_idx %s %s\n", s->base.name, i->base.name);
...
mvc_create_trigger(mvc *m, sql_table *t, const char *name, sht time, sht orientation, sht event, const char *old_name, const char *new_name, const char *condition, const char *statement )
{
	...
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_create_trigger %s %d %d %d\n", t->base.name, time, orientation, event);
...
{
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_drop_trigger %s %s\n", s->base.name, tri->base.name);
...
mvc_create_view(mvc *m, sql_schema *s, const char *name, int persistence, const char *sql, bit system)
{
	...
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_create_view %s %s %s\n", s->base.name, name, sql);
...
mvc_create_remote(mvc *m, sql_schema *s, const char *name, int persistence, const char *loc)
{
	...
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_create_remote %s %s %s\n", s->base.name, name, loc);
...
{
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_drop_table %s %s\n", s->base.name, t->base.name);
...
{
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_create_column %s %s %s\n", t->base.name, name, tpe->type->sqlname);
...
{
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_drop_column %s %s\n", t->base.name, col->base.name);
...
{
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_create_dependency %d %d %d\n", id, depend_id, depend_type);
...
mvc_create_dependencies(mvc *m, list *id_l, sqlid depend_id, sht dep_type)
{
	...
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_create_dependencies on %d of type %d\n", depend_id, dep_type);
...
mvc_check_dependency(mvc * m, sqlid id, sht type, list *ignore_ids)
{
	...
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_check_dependency on %d\n", id);
...
{
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_null %s %d\n", col->base.name, isnull);
...
{
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_default %s %s\n", col->base.name, val);
...
{
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_drop_default %s\n", col->base.name);
...
{
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_storage %s %s\n", col->base.name, storage);
...
{
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_access %s %d\n", t->base.name, access);
...
{
	if (mvc_debug)
		mnstr_printf(GDKerr, "#mvc_is_sorted %s\n", col->base.name);
