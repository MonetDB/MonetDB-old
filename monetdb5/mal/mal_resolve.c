...
findFunctionType(Module scope, MalBlkPtr mb, InstrPtr p, int silent)
{
	 ...
	 */
		...
		mnstr_printf(GDKerr,"#findFunction %s.%s\n", getModuleId(p), getFunctionId(p));
#endif
	m = scope;
...
/*  if(polyVector[0]==0) polyInit();
    memcpy(polytype,polyVector, 2*sig->argc*sizeof(int)); */
		...
		if (tracefcn && (sig->polymorphic || sig->retc == p->retc)) {
			mnstr_printf(GDKerr, "#resolving: ");
			fprintInstruction(GDKerr, mb, 0, p, LIST_MAL_ALL);
			mnstr_printf(GDKerr, "#against:");
			fprintInstruction(GDKerr, s->def, 0, getSignature(s), LIST_MAL_ALL);
		}
#endif
			for (k = 0; k < limit; k++)
				polytype[k] = TYPE_any;
		...
		if (tracefcn && (sig->polymorphic || sig->retc == p->retc)) {
			...
			for (i = p->retc; i < p->argc; i++) {
				...
				if (resolveType(formal, actual) == -1) {
					...
					mnstr_printf(GDKerr, "#unmatched %d formal %s actual %s\n",
								 i, ftpe, atpe);
					GDKfree(ftpe);
					GDKfree(atpe);
#endif
		...
		if (tracefcn) {
			...
			if( unmatched)
				mnstr_printf(GDKerr, "#unmatched %d test %s poly %s\n",
							 unmatched, tpe, tpe2);
			GDKfree(tpe);
			GDKfree(tpe2);
		}
		 ...
		 */
#ifdef DEBUG_MAL_RESOLVE
		mnstr_printf(GDKerr,"#TYPE RESOLVED:");
...
#ifdef DEBUG_MAL_RESOLVE
		if (tracefcn) {
			mnstr_printf(GDKerr, "#Wrapup matching returntype %d returns %d:",*returntype,*returns);
			fprintInstruction(GDKerr, mb, 0, p, LIST_MAL_ALL);
		}
#endif
	if (returntype != returns)
...
resolveType(int dsttype, int srctype)
{
	...
	if (tracefcn) {
		...
		mnstr_printf(GDKerr, "#resolveType dst %s (%d) %s(%d)\n",
	...
	if (isaBatType(dsttype) && isaBatType(srctype)) {
...
#ifdef DEBUG_MAL_RESOLVE
			if (tracefcn)
				mnstr_printf(GDKerr, "#Tail can not be resolved \n");
#endif
			return -1;
		}
#ifdef DEBUG_MAL_RESOLVE
		if (tracefcn) {
			...
			tpe2 = getTypeName(t2);
			tpe3 = getTypeName(t3);
			mnstr_printf(GDKerr, "#resolved to bat[:oid,:%s] bat[:oid,:%s]->bat[:oid,%s:%d]\n",
						 tpe1, tpe2, tpe3, i2);
			GDKfree(tpe1);
			GDKfree(tpe2);
			GDKfree(tpe3);
...
#ifdef DEBUG_MAL_RESOLVE
	if (tracefcn)
		mnstr_printf(GDKerr, "#Can not be resolved \n");
#endif
	return -1;
}
...
typeChecker(Module scope, MalBlkPtr mb, InstrPtr p, int silent)
{
	...
	if (getFunctionId(p) && getModuleId(p)) {
...
#ifdef DEBUG_MAL_RESOLVE
		mnstr_printf(GDKerr,"#typeChecker matched %d\n",s1);
#endif
		if (s1 >= 0)
			return;
		/*
		...
		if (!isaSignature(p) && !getInstrPtr(mb, 0)->polymorphic) {
			...
			p->typechk = TYPE_RESOLVED;
#ifdef DEBUG_MAL_RESOLVE
		mnstr_printf(GDKerr,"#typeChecker  no-sig and no-oly could not find it %d\n",p->typechk);
#endif
		return;
	}
	/*
	...
	if (getFunctionId(p)){
#ifdef DEBUG_MAL_RESOLVE
		mnstr_printf(GDKerr,"#typeChecker function call break %s\n", getFunctionId(p));
#endif
		return;
	}
...
#ifdef DEBUG_MAL_RESOLVE
				mnstr_printf(GDKerr,"#typeChecker function mismatch %s\n", getFunctionId(p));
#endif
				return;
			}
		} else {
...
updateTypeMap(int formal, int actual, int polytype[MAXTYPEVAR])
{
	...
	if(tracefcn){
		...
		mnstr_printf(GDKerr, "#updateTypeMap:formal %s actual %s\n", tpe1, tpe2);
		GDKfree(tpe1);
		GDKfree(tpe2);
	}
#endif
  ...
  updLabel:
#ifdef DEBUG_MAL_RESOLVE
	mnstr_printf(GDKerr, "#updateTypeMap returns: %d\n", ret);
#endif
	return ret;
}
