...
DFLOWworker(void *T)
{
	...
	if( GDKerrbuf == 0)
		mnstr_printf(GDKerr,"DFLOWworker:Could not allocate GDKerrbuf\n");
	else
		GDKclrerr();
	...
	while (1) {
		...
		PARDEBUG mnstr_printf(GDKerr, "#executed pc= %d wrk= %d claim= " LLFMT "," LLFMT "," LLFMT " %s\n",
...
DFLOWinitBlk(DataFlow flow, MalBlkPtr mb, int size)
{
	...
	PARDEBUG mnstr_printf(GDKerr, "#Initialize dflow block\n");
		...
		for (j = p->retc; j < p->argc; j++) {
			...
			if (!isVarConstant(mb, getArg(p, j))) {
				...
				if (l != pc && l < flow->stop && l > flow->start) {
					/* add edge to the target instruction for wakeup call */
					PARDEBUG mnstr_printf(GDKerr, "#endoflife for %s is %d -> %d\n", getVarName(mb, getArg(p, j)), n + flow->start, l);
	...
	PARDEBUG {
		for (n = 0; n < flow->stop - flow->start; n++) {
			mnstr_printf(GDKerr, "#[%d] %d: ", flow->start + n, n);
			fprintInstruction(GDKerr, mb, 0, getInstrPtr(mb, n + flow->start), LIST_MAL_ALL);
			mnstr_printf(GDKerr, "#[%d]Dependents block count %d wakeup", flow->start + n, flow->status[n].blocks);
			for (j = n; flow->edges[j]; j = flow->edges[j]) {
				mnstr_printf(GDKerr, "%d ", flow->start + flow->nodes[j]);
				if (flow->edges[j] == -1)
					break;
			}
			mnstr_printf(GDKerr, "\n");
...
static void showFlowEvent(DataFlow flow, int pc)
{
	...
	mnstr_printf(GDKerr, "#end of data flow %d done %d \n", pc, flow->stop - flow->start);
	for (i = 0; i < flow->stop - flow->start; i++)
		if (fe[i].state != DFLOWwrapup && fe[i].pc >= 0) {
			mnstr_printf(GDKerr, "#missed pc %d status %d %d  blocks %d", fe[i].state, i, fe[i].pc, fe[i].blocks);
...
DFLOWscheduler(DataFlow flow, struct worker *w)
{
	...
	for (i = 0; i < actions; i++)
		if (fe[i].blocks == 0) {
			...
			flow->status[i].state = DFLOWrunning;
			PARDEBUG mnstr_printf(GDKerr, "#enqueue pc=%d claim=" LLFMT "\n", flow->status[i].pc, flow->status[i].argclaim);
		}
	MT_lock_unset(&flow->flowlock);
	MT_sema_up(&w->s);

	PARDEBUG mnstr_printf(GDKerr, "#run %d instructions in dataflow block\n", actions);
	...
	if (flow->error ) {
		PARDEBUG mnstr_printf(GDKerr, "#errors encountered %s ", flow->error ? flow->error : "unknown");
		ret = flow->error;
	}
	return ret;
}
...
#ifdef DEBUG_FLOW
	mnstr_printf(GDKerr, "#runMALdataflow for block %d - %d\n", startpc, stoppc);
	fprintFunction(GDKerr, mb, 0, LIST_ALL);
#endif
