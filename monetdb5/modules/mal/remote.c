...
str RMTget(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci) {
	...
	if (isaBatType(rtype) && (localtype == 0177 || localtype != c->type ))
	{
...
#ifdef _DEBUG_REMOTE
		mnstr_printf(GDKerr, "#remote.get:%s\n", qbuf);
#else
		(void) cntxt;
#endif
		...
		{
#ifdef _DEBUG_REMOTE
			mnstr_printf(GDKerr, "#REMOTE GET error: %s\n%s\n",
					qbuf, tmp);
#endif
			MT_lock_unset(&c->lock);
		...
		snprintf(qbuf, BUFSIZ, "io.print(%s);", ident);
#ifdef _DEBUG_REMOTE
		mnstr_printf(GDKerr, "#remote:%s:%s\n", c->name, qbuf);
#endif
...
str RMTput(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci) {
	...
	if (type == TYPE_any || type == TYPE_bat || isAnyExpression(type)) {
		...
		GDKfree(val);
#ifdef _DEBUG_REMOTE
		mnstr_printf(GDKerr, "#remote.put:%s:%s\n", c->name, nbuf);
#endif
...
#ifdef _DEBUG_REMOTE
	mnstr_printf(GDKerr, "#remote.register:%s:%s\n", c->name, buf);
#endif
...
#ifdef _DEBUG_REMOTE
	mnstr_printf(GDKerr,"#remote.exec:%s:%s\n",c->name,qbuf);
#endif
