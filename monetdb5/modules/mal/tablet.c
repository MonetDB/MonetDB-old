...
TABLET_error(stream *s)
{
	if (!mnstr_errnr(GDKerr)) {
		...
		MT_fprintf(stderr, "#Stream error %s\n", err);
...
output_file_default(Tablet *as, BAT *order, stream *fd)
{
	...
	for (q = offset + as->nr, p = offset, id = order->hseqbase + offset; p < q; p++, id++) {
		...
		if ((i % 1000000) == 0)
			MT_fprintf(stderr, "#dumped " BUNFMT " lines\n", i);
#endif
	}
	GDKfree(localbuf);
	GDKfree(buf);
...
#ifdef _DEBUG_TABLET_
	MT_fprintf(stderr, "#tablet_error: " LLFMT ",%d:%s:%s\n", row, col, msg, fcn);
#endif
	task->errorcnt++;
...
#ifdef _DEBUG_TABLET_
	char *s;
	//MT_fprintf(stderr, "#SQL break line id %d  state %d\n%s", task->id, idx, line);
#endif
...
#ifdef _DEBUG_TABLET_
	if (error)
		MT_fprintf(stderr, "#line break failed %d:%s\n", idx, line ? line : "EOF");
#endif
	return error ? -1 : 0;
}
...
#ifdef _DEBUG_TABLET_
	MT_fprintf(stderr, "#SQLworker %d started\n", task->id);
#endif
...
#ifdef _DEBUG_TABLET_
			MT_fprintf(stderr, "#SQLworker id %d %d  piece %d-%d\n",
...
#ifdef _DEBUG_TABLET_
			MT_fprintf(stderr, "#SQLworker terminated\n");
#endif
			goto do_return;
		}
		MT_sema_up(&task->reply);
...
#ifdef _DEBUG_TABLET_
	MT_fprintf(stderr, "#SQLworker exits\n");
#endif
	MT_sema_up(&task->reply);
...
SQLproducer(void *p)
{
	...
	MT_fprintf(stderr, "#SQLproducer started size %zu len %zu\n",
	...
	for (;;) {
...
#ifdef _DEBUG_TABLET_CNTRL
		if (!ateof[cur])
			MT_fprintf(stderr, "#read %zu bytes pos = %zu eof=%d offset=" LLFMT " \n",
...
#ifdef _DEBUG_TABLET_CNTRL
				MT_fprintf(stderr, "#bailout on SQLload %s\n", msg);
#endif
				ateof[cur] = true;
				break;
			}
	  ...
	  parseSTDIN:
#ifdef _DEBUG_TABLET_
		if (!ateof[cur])
			MT_fprintf(stderr, "#parse input:%.63s\n",
		...
		if (end - s + task->b->len - task->b->pos >= task->rowlimit[cur]) {
			...
			ateof[cur] = true;
#ifdef _DEBUG_TABLET_CNTRL
			MT_fprintf(stderr, "#bailout on SQLload confronted with too large record\n");
#endif
			goto reportlackofinput;
		}
	  ...
	  reportlackofinput:
#ifdef _DEBUG_TABLET_CNTRL
		MT_fprintf(stderr, "#SQL producer got buffer %d filled with %d records \n",
					 cur, task->top[cur]);
#endif
		if (consoleinput) {
			task->cur = cur;
...
#ifdef _DEBUG_TABLET_CNTRL
				MT_fprintf(stderr, "#wait for consumers to finish buffer %d\n",
							 (cur + 1) % MAXBUFFERS);
#endif
				MT_sema_down(&task->producer);
...
#ifdef _DEBUG_TABLET_CNTRL
			MT_fprintf(stderr, "#May continue with buffer %d\n", cur);
#endif
			if (cnt == task->maxrow) {
				MT_sema_down(&task->producer);
#ifdef _DEBUG_TABLET_CNTRL
				MT_fprintf(stderr, "#Producer delivered all\n");
#endif
				GDKfree(rdfa);
				return;
			}
...
#ifdef _DEBUG_TABLET_CNTRL
		MT_fprintf(stderr, "#Continue producer buffer %d\n", cur);
#endif
		/* we ran out of input? */
		if (task->ateof) {
#ifdef _DEBUG_TABLET_CNTRL
			MT_fprintf(stderr, "#Producer encountered eof\n");
#endif
			GDKfree(rdfa);
			return;
		}
...
#ifdef _DEBUG_TABLET_CNTRL
			if (!ateof[cur])
				MT_fprintf(stderr, "#SQL producer early exit %.63s\n",
							 task->b->buf + task->b->pos);
#endif
			GDKfree(rdfa);
			return;
...
#ifdef _DEBUG_TABLET_CNTRL
		if (!ateof[cur])
			MT_fprintf(stderr, "#shuffle %zu: %.63s\n", strlen(s), s);
#endif
...
SQLload_file(Client cntxt, Tablet *as, bstream *b, stream *out, const char *csep, const char *rsep, char quote, lng skip, lng maxrow, int best, bool from_stdin, const char *tabnam)
{
...
#ifdef _DEBUG_TABLET_
	MT_fprintf(stderr, "#Prepare copy work for %d threads col '%s' rec '%s' quot '%c'\n",
...
#ifdef _DEBUG_TABLET_
	MT_fprintf(stderr, "#parallel bulk load " LLFMT " - " BUNFMT "\n", skip, task.maxrow);
#endif
	...
	while (res == 0 && cnt < task.maxrow) {
		...
		iototal += tio;
#ifdef _DEBUG_TABLET_
		MT_fprintf(stderr, "#Break %d lines\n", task.top[task.cur]);
#endif
		t1 = GDKusec();
		...
		if (task.top[task.cur]) {
			...
			for (j = 0; j < threads; j++) {
				...
				if (ptask[j].error) {
					res = -1;
#ifdef _DEBUG_TABLET_
					MT_fprintf(stderr, "#Error in task %d %d\n", j, ptask[j].error);
#endif
				}
			}
		}
#ifdef _DEBUG_TABLET_
		MT_fprintf(stderr, "#fill the BATs %d  " BUNFMT " cap " BUNFMT "\n",
...
#define trimerrors(TYPE)												\
...
#ifdef _DEBUG_TABLET_
		MT_fprintf(stderr, "#Trim bbest %d table size " BUNFMT " rows found so far " BUNFMT "\n",
...
#ifdef _DEBUG_TABLET_
	MT_fprintf(stderr, "#Enf of block stream eof=%d res=%d\n",
				 task.ateof, res);
#endif

	cnt = BATcount(task.as->format[firstcol].c);
	if (GDKdebug & GRPalgorithms) {
		MT_fprintf(stderr, "#COPY reader time " LLFMT " line break " LLFMT " io " LLFMT "\n",
					 total, lio, iototal);
#ifdef _DEBUG_TABLET_
		for (i = 0; i < as->nr_attrs; i++)
			MT_fprintf(stderr, LLFMT " ", task.time[i]);
		MT_fprintf(stderr, "\n");
		for (j = 0; j < threads; j++)
			MT_fprintf(stderr, "#COPY thread time " LLFMT "\n", ptask[j].wtime);
#endif
	}
	...
	task.state = ENDOFCOPY;
#ifdef _DEBUG_TABLET_
	MT_fprintf(stderr, "#Activate sync on disk \n");
#endif
...
#ifdef _DEBUG_TABLET_
		MT_fprintf(stderr, "#Shut down reader\n");
#endif
		MT_sema_up(&task.producer);
	}
...
#ifdef _DEBUG_TABLET_
	MT_fprintf(stderr, "#Activate endofcopy\n");
#endif
	for (j = 0; j < threads; j++) {
...
#ifdef _DEBUG_TABLET_
	MT_fprintf(stderr, "#Kill the workers\n");
#endif
	for (j = 0; j < threads; j++) {
...
#ifdef _DEBUG_TABLET_
	MT_fprintf(stderr, "#Found " BUNFMT " tuples\n", cnt);
	MT_fprintf(stderr, "#leftover input:%.63s\n",
				 task.b->buf + task.b->pos);
#endif
	for (i = 0; i < as->nr_attrs; i++) {
