...
WLCsetlogger(void)
{
	...
	if( wlc_fd == 0){
		MT_lock_unset(&wlc_lock);
		MT_fprintf(stderr, "wlc.logger:Could not create %s\n",path);
...
WLCinit(void)
{
	...
	if( wlc_state == WLC_STARTUP){
		...
		if (MT_create_thread(&wlc_logger, WLClogger , (void*) 0,
			...
			MT_fprintf(stderr, "wlc.logger thread could not be spawned");
		}
	}
	return MAL_SUCCEED;
}
...
{	str msg = MAL_SUCCEED;
...
#ifdef _WLC_DEBUG_
	if( cntxt->wlc){
		MT_fprintf(stderr,"#WLCpreparewrite: %d %d\n", cntxt->wlc->stop , cntxt->wlc_kind);
	...
	if( wlc_state != WLC_RUN){
#ifdef _WLC_DEBUG_
		MT_fprintf(stderr,"#WLCprepare: state %d\n", wlc_state);
#endif
	...
	if( wlc_dir[0] ){	
		if (wlc_fd == NULL){
			...
			if( msg) {
#ifdef _WLC_DEBUG_
				MT_fprintf(stderr,"#WLCprepare: setlogger %s \n", msg);
#endif
				return msg;
			}
		}
...
// be accessible in the replica. TODO
	...
	case TYPE_str: 
		...
		break;
	default:
		MT_fprintf(stderr, "#wlc datashipping, non-supported type %d\n", ATOMstorage(b->ttype));
		cntxt->wlc_kind = WLC_CATALOG;
	}
finish:
