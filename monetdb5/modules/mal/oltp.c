...
#ifdef _DEBUG_OLTP_
	mnstr_printf(GDKerr,"#OLTP %6d reset locktable\n", GDKms());
#endif
	for( i=0; i<MAXOLTPLOCKS; i++){
...
OLTPenable(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	(void) cntxt;
#ifdef _DEBUG_OLTP_
	mnstr_printf(GDKerr,"#OLTP %6d enabled\n",GDKms());
#endif
	oltp_delay = TRUE;
	return MAL_SUCCEED;
}
...
OLTPdisable(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	oltp_delay = FALSE;
#ifdef _DEBUG_OLTP_
	mnstr_printf(GDKerr,"#OLTP %6d disabled\n",GDKms());
#else
	(void) cntxt;
#endif
	return MAL_SUCCEED;
...
#ifdef _DEBUG_OLTP_
	mnstr_printf(GDKerr,"#OLTP %6d lock request for client %d:", GDKms(), cntxt->idx);
	fprintInstruction(GDKerr,mb,stk,pci, LIST_MAL_ALL);
#endif
	do{
		MT_lock_set(&mal_oltpLock);
		clk = GDKms();
		...
		if( i  == pci->argc ){
#ifdef _DEBUG_OLTP_
			mnstr_printf(GDKerr,"#OLTP %6d set lock for client %d\n", GDKms(), cntxt->idx);
#endif
			for( i=1; i< pci->argc; i++){
				lck= getVarConstant(mb, getArg(pci,i)).val.ival;
			...
			MT_lock_unset(&mal_oltpLock);
#ifdef _DEBUG_OLTP_
			mnstr_printf(GDKerr,"#OLTP %d delay imposed for client %d\n", GDKms(), cntxt->idx);
#endif
...
#ifdef _DEBUG_OLTP_
	mnstr_printf(GDKerr,"#OLTP %6d proceed query for client %d\n", GDKms(), cntxt->idx);
#endif
...
#ifdef _DEBUG_OLTP_
			mnstr_printf(GDKerr,"#OLTP %6d bail out a concurrent copy into %d\n", GDKms(), cntxt->idx);
#endif
...
OLTPrelease(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	clk = GDKms();
#ifdef _DEBUG_OLTP_
	mnstr_printf(GDKerr,"#OLTP %6d release the locks %d:", GDKms(), cntxt->idx);
	fprintInstruction(GDKerr,mb,stk,pci, LIST_MAL_ALL);
#endif
	for( i=1; i< pci->argc; i++){
		lck= getVarConstant(mb, getArg(pci,i)).val.ival;
...
#ifdef _DEBUG_OLTP_
				mnstr_printf(GDKerr,"#OLTP  retention period for lock %d: "LLFMT"\n", lck,delay);
#endif
			}
	}
	MT_lock_unset(&mal_oltpLock);
