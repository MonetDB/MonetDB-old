...
timestamp_trunc_after_every(timestamp *ret, timestamp *t, str *field, int *after_every)
{
	...
	if ((e = MTIMEtimestamp_extract_date(&dayys, t, &tzone_local)) != MAL_SUCCEED)
		return e;
	if ((e = MTIMEtimestamp_extract_daytime(&mseccs, t, &tzone_local)) != MAL_SUCCEED)
		return e;
	
	// resolve date and daytime into its components
	fromdate(dayys, &day, &month, &year);
	fromtime(mseccs, &hour, &min, &sec, &msec);
	...
	if ((e = MTIMEdate_create(&dayys, &year, &month, &day)) != MAL_SUCCEED)
		return e;
	if ((e = MTIMEdaytime_create(&mseccs, &hour, &min, &sec, &msec)) != MAL_SUCCEED)
		return e;
	
	// combine new date and daytime into timestamp
	if ((e = MTIMEtimestamp_create_default(ret, &dayys, &mseccs)) != MAL_SUCCEED)
		return e;
...
timestamp_trunc_after_every_bulk(int *ret, int *bid, str *field, int *after_every)
{
	...
	bn = BATnew(TYPE_void, TYPE_timestamp, BATcount(b), TRANSIENT);
	if (bn == NULL)
	...
	BATloop(b, p, q) {
		...
		if (BUNappend(bn, &res, FALSE) == GDK_FAIL) {
			BBPunfix(bn->batCacheid);
