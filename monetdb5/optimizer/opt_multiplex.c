...
OPTexpandMultiplex(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	MT_fprintf(stderr,"#WARNING To speedup %s.%s a bulk operator implementation is needed\n#", mod,fcn);
	...
	{	char *tpenme;
		MT_fprintf(stderr,"#calling the optimize multiplex script routine\n");
		fprintFunction(stderr,mb, 0, LIST_MAL_ALL );
		tpenme = getTypeName(getVarType(mb,iter));
		MT_fprintf(stderr,"#multiplex against operator %d %s\n",iter, tpenme);
		GDKfree(tpenme);
...
OPTmultiplexImplementation(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
    ...
    if( OPTdebug &  OPTmultiplex){
        MT_fprintf(stderr, "#MULTIPLEX optimizer exit\n");
        fprintFunction(stderr, mb, 0,  LIST_MAL_ALL);
    }
	return msg;
}
