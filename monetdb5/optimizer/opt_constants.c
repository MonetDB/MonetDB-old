...
OPTconstantsImplementation(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr p)
{
	...
	mnstr_printf(GDKerr,"#OPT_CONSTANTS: MATCHING CONSTANTS ELEMENTS\n");
#endif
	...
	for (i=0; i< mb->vtop && n < 100; i++)
		if ( isVarConstant(mb,i)  && isVarFixed(mb,i)  && getVarType(mb,i) != TYPE_ptr){
...
#ifdef DEBUG_OPT_CONSTANTS
					mnstr_printf(GDKerr,"#opt_constants: matching elements %s %d %d\n", getVarName(mb,i), i,k);
#endif
					/* re-use a constant */
					alias[i]= index[k];
					fnd=1;
			...
			if ( fnd == 0){
#ifdef DEBUG_OPT_CONSTANTS
				mnstr_printf(GDKerr,"swith elements %d %d\n", i,n);
#endif
				cst[n]= x;
				index[n]= i;
				n++;
