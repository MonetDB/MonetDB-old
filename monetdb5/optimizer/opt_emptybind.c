...
OPTemptybindImplementation(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
    ...
    if( OPTdebug & OPTemptybind ){
		TraceLN(M_DEBUG, func_ln, "EMPTYBIND optimizer entry\n");
	...
	for (i = 0; i < limit; i++) {
		...
		if ( getModuleId(p) == batRef && getFunctionId(p) == newRef){
			if( OPTdebug &  OPTemptybind){
				TraceLN(M_DEBUG, func_ln, "Empty bat pc %d var %d\n", i, getArg(p,0));
			}
			empty[getArg(p,0)] = i;
			continue;
		} 
		...
		if (getFunctionId(p) == emptybindRef) {
			if( OPTdebug &  OPTemptybind){
				TraceLN(M_DEBUG, func_ln, "Empty bind pc %d var %d\n", i, getArg(p,0));
			}
			...
			if( p->retc == 2){
				empty[getArg(p,1)] = i;
				if( OPTdebug &  OPTemptybind){
					TraceLN(M_DEBUG, func_ln, "Empty update bind pc %d var %d\n", i, getArg(p,1));
				}
			}
			...
			for(j= 0; j< etop; j++){
				...
				if(q && getModuleId(q) == sqlRef && isUpdateInstruction(q)){
					if ( strcmp(getVarConstant(mb,getArg(q,2)).val.sval, sch) == 0 &&
						...
						if( OPTdebug &  OPTemptybind){
							TraceLN(M_DEBUG, func_ln, "Reset mark empty variable pc %d var %d\n", i, getArg(p,0));
						}
						empty[getArg(p,0)] = 0;
						if( p->retc == 2){
							if( OPTdebug &  OPTemptybind){
		...
		if (getFunctionId(p) == emptybindidxRef) {
			if( OPTdebug &  OPTemptybind){
				TraceLN(M_DEBUG, func_ln, "Empty bindidx pc %d var %d\n", i, getArg(p,0));
			}
		...
		if (getModuleId(p)== sqlRef && getFunctionId(p) == deltaRef  && p->argc ==5){
			if( empty[getArg(p,2)] && empty[getArg(p,3)] && empty[getArg(p,4)] ){
				if( OPTdebug &  OPTemptybind){
					TraceLN(M_DEBUG, func_ln, "Empty delta pc %d var %d,%d,%d\n", i, empty[getArg(p,2)], empty[getArg(p,3)], empty[getArg(p,4)]);
					TraceLN(M_DEBUG, func_ln, "Empty delta pc %d var %d\n", i, getArg(p,0));
				}
				actions++;
				clrFunction(p);
				p->argc = 2;
		...
		if (getModuleId(p)== sqlRef && getFunctionId(p) == projectdeltaRef) {
			if( empty[getArg(p,3)] && empty[getArg(p,4)] ){
				if( OPTdebug &  OPTemptybind){
					TraceLN(M_DEBUG, func_ln, "Empty projectdelta pc %d var %d\n", i, getArg(p,0));
				}
				actions++;
		...
		if (getModuleId(p)== algebraRef){
			if( getFunctionId(p) == projectionRef) {
				if( empty[getArg(p,1)] || empty[getArg(p,2)] ){
					if( OPTdebug &  OPTemptybind){
						TraceLN(M_DEBUG, func_ln, "Empty projection pc %d var %d\n", i, getArg(p,0));
					}
					actions++;
					emptyresult(0);
				}
			...
			if( getFunctionId(p) == thetaselectRef || getFunctionId(p) == selectRef) {
				if( empty[getArg(p,1)] || empty[getArg(p,2)] ){
					if( OPTdebug &  OPTemptybind){
						TraceLN(M_DEBUG, func_ln, "Empty selection pc %d var %d\n", i, getArg(p,0));
					}
					actions++;
					emptyresult(0);
				}
		...
		if( getModuleId(p) == batstrRef){
			if( empty[getArg(p,1)] || empty[getArg(p,2)] ){
				...
				if( OPTdebug &  OPTemptybind){
					TraceLN(M_DEBUG, func_ln, "Empty string operation pc %d var %d\n", i, getArg(p,0));
				}
				actions++;
				emptyresult(0);
			}
...
wrapup:
    ...
    if( OPTdebug &  OPTemptybind){
        TraceLN(M_DEBUG, func_ln, "EMPTYBIND optimizer exit\n");
        fprintFunction(M_DEBUG, func_ln, mb, 0, LIST_MAL_ALL);
    }
	return msg;
}
