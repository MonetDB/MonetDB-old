...
OPTemptybindImplementation(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	MT_fprintf(stderr, "#Optimize Query Emptybind\n");
	...
	for (i = 0; i < limit; i++) {
...
#ifdef DEBUG_OPT_EMPTYBIND
			MT_fprintf(stderr, "#empty bat  pc %d var %d\n",i , getArg(p,0) );
#endif
			empty[getArg(p,0)] = i;
			continue;
		} 
		...
		if (getFunctionId(p) == emptybindRef) {
#ifdef DEBUG_OPT_EMPTYBIND
			MT_fprintf(stderr, "#empty bind  pc %d var %d\n",i , getArg(p,0) );
#endif
			...
			if( p->retc == 2){
				empty[getArg(p,1)] = i;
#ifdef DEBUG_OPT_EMPTYBIND
				MT_fprintf(stderr, "#empty update bind  pc %d var %d\n",i , getArg(p,1) );
#endif
			}
...
#ifdef DEBUG_OPT_EMPTYBIND
						MT_fprintf(stderr, "#reset mark empty variable pc %d var %d\n",i , getArg(p,0) );
#endif
						empty[getArg(p,0)] = 0;
						if( p->retc == 2){
#ifdef DEBUG_OPT_EMPTYBIND
...
#ifdef DEBUG_OPT_EMPTYBIND
			MT_fprintf(stderr, "#empty bindidx  pc %d var %d\n",i , getArg(p,0) );
#endif
...
#ifdef DEBUG_OPT_EMPTYBIND
				MT_fprintf(stderr, "#empty delta  pc %d var %d,%d,%d\n",i ,empty[getArg(p,2)], empty[getArg(p,3)], empty[getArg(p,4)] );
				MT_fprintf(stderr, "#empty delta  pc %d var %d\n",i , getArg(p,0) );
#endif
				actions++;
				clrFunction(p);
				p->argc = 2;
...
#ifdef DEBUG_OPT_EMPTYBIND
				MT_fprintf(stderr, "#empty projectdelta  pc %d var %d\n",i , getArg(p,0) );
#endif
				actions++;
		...
		if (getModuleId(p)== algebraRef){
...
#ifdef DEBUG_OPT_EMPTYBIND
					MT_fprintf(stderr, "#empty projection  pc %d var %d\n",i , getArg(p,0) );
#endif
					actions++;
					emptyresult(0);
				}
			...
			if( getFunctionId(p) == thetaselectRef || getFunctionId(p) == selectRef) {
				if( empty[getArg(p,1)] || empty[getArg(p,2)] ){
#ifdef DEBUG_OPT_EMPTYBIND
					MT_fprintf(stderr, "#empty selection  pc %d var %d\n",i , getArg(p,0) );
#endif
					actions++;
					emptyresult(0);
				}
		...
		if( getModuleId(p) == batstrRef){
			if( empty[getArg(p,1)] || empty[getArg(p,2)] ){
#ifdef DEBUG_OPT_EMPTYBIND
				MT_fprintf(stderr, "#empty string operation  pc %d var %d\n",i , getArg(p,0) );
#endif
				actions++;
				emptyresult(0);
			}
	...
	MT_fprintf(stderr, "#Optimize Query Emptybind done\n");
	fprintFunction(stderr, mb, 0, LIST_MAL_DEBUG);
#endif
