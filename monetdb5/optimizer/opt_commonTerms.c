	...
	for ( i = 0; i < limit; i++) {
		...
		for ( k = 0; k < p->retc; k++)
			if( vars[getArg(p,k)] && p->barrier != RETURNsymbol){
				...
				mnstr_printf(GDKerr, "#ERROR MULTIPLE ASSIGNMENTS[%d] ",i);
		...
		if (p->token == ENDsymbol){
			...
			for(i++; i<limit; i++)
				if( old[i]){
#ifdef DEBUG_OPT_COMMONTERMS_MORE
					mnstr_printf(GDKerr, "#FINALIZE[%d] ",i);
					fprintInstruction(GDKerr, mb, 0, old[i], LIST_MAL_ALL);
#endif
...
#ifdef DEBUG_OPT_COMMONTERMS_MORE
				mnstr_printf(GDKerr, "#COMMON SKIPPED[%d] %d %d\n",i, barrier, p->retc == p->argc);
#endif
			continue;
		}
...
#ifdef DEBUG_OPT_COMMONTERMS_MORE
		mnstr_printf(GDKerr,"#TARGET CANDIDATE[%d] ",i);
		fprintInstruction(GDKerr, mb, 0, p, LIST_MAL_ALL);
#endif
...
#ifdef DEBUG_OPT_COMMONTERMS_MORE
			mnstr_printf(GDKerr,"#CANDIDATE[%d->%d] %d %d", j, list[j],
...
#ifdef DEBUG_OPT_COMMONTERMS_MORE
						mnstr_printf(GDKerr,"#safetybarrier reached\n");
#endif
						break;
					}
					clrFunction(p);
...
#ifdef DEBUG_OPT_COMMONTERMS_MORE
					mnstr_printf(GDKerr, "#MODIFIED EXPRESSION %d -> %d ",getArg(p,0),getArg(p,1));
					fprintInstruction(GDKerr, mb, 0, p, LIST_MAL_ALL);
#endif
					actions++;
					break; /* end of search */
				...
				mnstr_printf(GDKerr, "#COMMON SKIPPED %d %d ", mayhaveSideEffects(cntxt, mb, q, TRUE) , isUpdateInstruction(p));
