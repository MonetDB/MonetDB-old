	...
	for ( i = 0; i < limit; i++) {
		...
		for ( k = 0; k < p->retc; k++)
			if( vars[getArg(p,k)] && p->barrier != RETURNsymbol){
				...
				MT_fprintf(stderr, "#ERROR MULTIPLE ASSIGNMENTS[%d] ",i);
		...
		if (p->token == ENDsymbol){
			...
			for(i++; i<limit; i++)
				if( old[i]){
#ifdef DEBUG_OPT_COMMONTERMS_MORE
					MT_fprintf(stderr, "#FINALIZE[%d] ",i);
...
#ifdef DEBUG_OPT_COMMONTERMS_MORE
		MT_fprintf(stderr,"#TARGET CANDIDATE[%d] ",i);
		fprintInstruction(stderr, mb, 0, p, LIST_MAL_ALL);
#endif
...
#ifdef DEBUG_OPT_COMMONTERMS_MORE
			MT_fprintf(stderr,"#CANDIDATE[%d->%d] %d %d", j, list[j],
