...
-- 	"file_location" STRING,
	...
	"sample_count"   BIGINT,
	"sample_type"   CHAR(1),
	CONSTRAINT "segments_file_id_segment_id_pkey" PRIMARY KEY (file_id, segment_id),
	CONSTRAINT "segments_fkey_files_file_id" FOREIGN KEY (file_id) REFERENCES mseed.files(file_id)
...
SELECT f.file_id, file_location, dataquality, network, station, location, channel, encoding, byte_order, segment_id, start_time, end_time, prev_gap, frequency, sample_count, sample_type
FROM mseed.files AS f
	JOIN mseed.segments AS s
		ON f.file_id = s.file_id;

CREATE VIEW mseed.dataview AS
SELECT f.file_id, file_location, dataquality, network, station, location, channel, encoding, byte_order, s.segment_id, start_time, end_time, prev_gap, frequency, sample_count, sample_type, sample_time, sample_value
FROM mseed.files AS f 
	JOIN mseed.segments AS s
		ON f.file_id = s.file_id
	JOIN mseed.data AS d 
		ON s.file_id = d.file_id AND s.segment_id = d.segment_id;
