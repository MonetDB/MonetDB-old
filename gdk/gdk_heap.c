...
HEAPalloc(Heap *h, size_t nitems, size_t itemsize)
{
	...
	if (GDKinmemory() ||
		...
		HEAPDEBUG mnstr_printf(GDKerr, "#HEAPalloc %zu %p\n", h->size, h->base);
	}
...
HEAPextend(Heap *h, size_t size, bool mayshare)
{
	...
	failure = "size > h->size";
 	...
 	if (h->storage != STORE_MEM) {
		...
		HEAPDEBUG mnstr_printf(GDKerr, "#HEAPextend: extending %s mmapped heap (%s)\n", h->storage == STORE_MMAP ? "shared" : "privately", h->filename);
		...
		if (!must_mmap) {
			...
			HEAPDEBUG mnstr_printf(GDKerr, "#HEAPextend: extending malloced heap %zu %zu %p %p\n", size, h->size, bak.base, h->base);
			h->size = size;
			if (h->base)
				return GDK_SUCCEED; /* success */
...
HEAPshrink(Heap *h, size_t size)
{
	...
	if (h->storage == STORE_MEM) {
		...
		HEAPDEBUG mnstr_printf(GDKerr, "#HEAPshrink: shrinking malloced "
				  "heap %zu %zu %p "
				  "%p\n", h->size, size,
				  h->base, p);
	} else {
		...
		p = GDKmremap(path,
				...
				MMAP_READ | MMAP_WRITE,
			      h->base, h->size, &size);
		GDKfree(path);
		HEAPDEBUG mnstr_printf(GDKerr, "#HEAPshrink: shrinking %s mmapped "
				  "heap (%s) %zu %zu %p "
				  "%p\n",
...
HEAPfree(Heap *h, bool rmheap)
{
	if (h->base) {
		if (h->storage == STORE_MEM) {	/* plain memory */
			HEAPDEBUG mnstr_printf(GDKerr, "#HEAPfree %zu"
					  " %p\n",
					  h->size, h->base);
			GDKfree(h->base);
			...
			HEAPDEBUG mnstr_printf(GDKerr, "#munmap(base=%p, "
					  "size=%zu) = %d\n",
					  (void *)h->base,
					  h->size, (int) ret);
		}
...
HEAPload_intern(Heap *h, const char *nme, const char *ext, const char *suffix, bool trunc)
{
	...
	HEAPDEBUG mnstr_printf(GDKerr, "#HEAPload(%s.%s,storage=%d,free=%zu"
			  ",size=%zu)\n", nme, ext,
	...
	HEAPDEBUG mnstr_printf(GDKerr, "#rename %s %s = %d %s (%dms)\n",
...
HEAPsave_intern(Heap *h, const char *nme, const char *ext, const char *suffix)
{
	...
	HEAPDEBUG {
		mnstr_printf(GDKerr, "#HEAPsave(%s.%s,storage=%d,free=%zu,size=%zu)\n", nme, ext, (int) h->newstorage, h->free, h->size);
	}
...
HEAP_empty(Heap *heap, size_t nprivate, int alignment)
{
	...
	headp->next = 0;
#ifdef TRACE
	mnstr_printf(GDKerr, "#We created the following heap\n");
...
#ifdef TRACE
	mnstr_printf(GDKerr, "#Enter malloc with %zu bytes\n", nbytes);
#endif
...
#ifdef TRACE
		mnstr_printf(GDKerr, "#block %zu is %zu bytes\n", block, blockp->size);
#endif
...
#ifdef TRACE
		mnstr_printf(GDKerr, "#No block found\n");
#endif

		/* Increase the size of the heap. */
		HEAPDEBUG mnstr_printf(GDKerr, "#HEAPextend in HEAP_malloc %s %zu %zu\n", heap->filename, heap->size, newsize);
...
#ifdef TRACE
		mnstr_printf(GDKerr, "#New block made at pos %zu with size %zu\n", block, heap->size - block);
#endif
...
#ifdef TRACE
			mnstr_printf(GDKerr, "#Glue newly generated block to adjacent last\n");
#endif
