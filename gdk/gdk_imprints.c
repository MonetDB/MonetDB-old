...
BATcheckimprints(BAT *b)
{
		...
		if ((imprints = GDKzalloc(sizeof(Imprints))) != NULL &&
			...
			if ((fd = GDKfdlocate(imprints->imprints.farmid, nme, "rb", "timprints")) >= 0) {
				...
				if (read(fd, hdata, sizeof(hdata)) == sizeof(hdata) &&
					...
					ALGODEBUG MT_fprintf(stderr, "#BATcheckimprints(" ALGOBATFMT "): reusing persisted imprints\n", ALGOBATPAR(b));
	...
	MT_lock_unset(&GDKimprintsLock(b->batCacheid));
	ALGODEBUG if (ret) MT_fprintf(stderr, "#BATcheckimprints(" ALGOBATFMT "): already has imprints\n", ALGOBATPAR(b));
	return ret;
}
...
BATimpsync(void *arg)
{
	...
	if ((imprints = b->timprints) != NULL) {
		...
		if (HEAPsave(hp, hp->filename, NULL) == GDK_SUCCEED) {
			...
			ALGODEBUG MT_fprintf(stderr, "#BATimpsync(" ALGOBATFMT "): "
					  "imprints persisted "
					  "(" LLFMT " usec)%s\n", ALGOBATPAR(b),
					  GDKusec() - t0, failed);
		}
...
#define SMP_SIZE 2048
	...
	ALGODEBUG MT_fprintf(stderr, "#BATimprints(%s): imprints construction " LLFMT " usec\n", BATgetId(b), GDKusec() - t0);
	MT_lock_unset(&GDKimprintsLock(b->batCacheid));
