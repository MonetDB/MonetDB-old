...
logbat_new(int tt, BUN size, role_t role)
{
	...
	if (nb) {
		...
		MT_fprintf(stderr, "!ERROR: logbat_new: creating new BAT[void:%s]#" BUNFMT " failed\n", ATOMname(tt), size);
	}
	return nb;
}
...
log_write_format(logger *l, logformat *data)
{
	if (mnstr_write(l->log, &data->flag, 1, 1) == 1 &&
		...
		return GDK_SUCCEED;
	MT_fprintf(stderr, "!ERROR: log_write_format: write failed\n");
	return GDK_FAIL;
}
...
log_read_string(logger *l)
{
	...
	if (mnstr_readInt(l->log, &len) != 1) {
		MT_fprintf(stderr, "!ERROR: log_read_string: read failed\n");
//MK This leads to non-repeatable log structure?
	...
	buf = GDKmalloc(len);
	if (buf == NULL) {
		MT_fprintf(stderr, "!ERROR: log_read_string: malloc failed\n");
		/* this is bad */
		return (char *) -1;
	}
	...
	if ((nr = mnstr_read(l->log, buf, 1, len)) != (ssize_t) len) {
		buf[len - 1] = 0;
		MT_fprintf(stderr, "!ERROR: log_read_string: couldn't read name (%s) %zd\n", buf, nr);
		GDKfree(buf);
		return NULL;
	}
	buf[len - 1] = 0;
...
log_write_string(logger *l, const char *n)
{
	...
	if (!mnstr_writeInt(l->log, (int) len) ||
		...
		MT_fprintf(stderr, "!ERROR: log_write_string: write failed\n");
		return GDK_FAIL;
	}
	return GDK_SUCCEED;
}
...
{
	if (lg->debug & 1)
		MT_fprintf(stderr, "#logger found log_read_clear %s\n", NAME(name, tpe, id));
...
la_bat_clear(logger *lg, logaction *la)
{
	...
	if (lg->debug & 1)
		MT_fprintf(stderr, "#la_bat_clear %s\n", NAME(la->name, la->tpe, la->cid));
...
log_read_seq(logger *lg, logformat *l)
{
	...
	if (mnstr_readLng(lg->log, &val) != 1) {
		MT_fprintf(stderr, "!ERROR: log_read_seq: read failed\n");
		return LOG_EOF;
	}
...
log_write_id(logger *l, char tpe, oid id)
{
	...
	if (mnstr_writeChr(l->log, tpe) &&
		...
		return GDK_SUCCEED;
	MT_fprintf(stderr, "!ERROR: log_write_id: write failed\n");
	return GDK_FAIL;
}
...
log_read_id(logger *lg, char *tpe, oid *id)
{
	...
	if (mnstr_readChr(lg->log, tpe) != 1 ||
		...
		MT_fprintf(stderr, "!ERROR: log_read_id: read failed\n");
		return LOG_EOF;
	}
	*id = (oid)lid;
	return LOG_OK;
...
log_read_updates(logger *lg, trans *tr, logformat *l, char *name, int tpe, oid id)
{
	...
	if (lg->debug & 1)
		MT_fprintf(stderr, "#logger found log_read_updates %s %s " LLFMT "\n", name, l->flag == LOG_INSERT ? "insert" : "update", l->nr);
...
log_read_create(logger *lg, trans *tr, char *name, char tpe, oid id)
{
	...
	if (lg->debug & 1)
		MT_fprintf(stderr, "#log_read_create %s\n", name);
	...
	ta = strchr(buf, ',');
	if (ta == NULL) {
		MT_fprintf(stderr, "!ERROR: log_read_create: inconsistent data read\n");
		return LOG_ERR;
	}
	*ta++ = 0;		/* skip over , */
	if (strcmp(ha, "vid") == 0) {
...
logger_open(logger *lg)
{
	...
	if (lg->log == NULL || mnstr_errnr(lg->log)) {
		MT_fprintf(stderr, "!ERROR: logger_open: creating %s failed\n", filename);
		GDKfree(filename);
		return GDK_FAIL;
	}
	GDKfree(filename);
...
logger_readlog(logger *lg, char *filename, bool *filemissing)
{
	...
	if (lg->debug & 1) {
		MT_fprintf(stderr, "#logger_readlog opening %s\n", filename);
	}
	...
	if ((fd = getFileNo(lg->log)) < 0 || fstat(fd, &sb) < 0) {
		MT_fprintf(stderr, "!ERROR: logger_readlog: fstat on opened file %s failed\n", filename);
		close_stream(lg->log);
		lg->log = NULL;
		GDKdebug = dbg;
	...
	while (err == LOG_OK && log_read_format(lg, &l)) {
		...
		if (lg->debug & 1) {
			MT_fprintf(stderr, "#logger_readlog: ");
			if (l.flag > 0 &&
			    l.flag < (char) (sizeof(log_commands) / sizeof(log_commands[0])))
				MT_fprintf(stderr, "%s", log_commands[(int) l.flag]);
			else
				MT_fprintf(stderr, "%d", l.flag);
			MT_fprintf(stderr, " %d " LLFMT, l.tid, l.nr);
			if (name)
				MT_fprintf(stderr, " %s", name);
			MT_fprintf(stderr, "\n");
		}
		...
		case LOG_START:
			...
			if (lg->debug & 1)
				MT_fprintf(stderr, "#logger tstart %d\n", tr->tid);
			break;
		case LOG_END:
			if (tr == NULL)
				err = LOG_EOF;
...
logger_readlogs(logger *lg, FILE *fp, char *filename)
{
	...
	if (lg->debug & 1) {
		MT_fprintf(stderr, "#logger_readlogs logger id is " LLFMT "\n", lg->id);
	}
	...
	if (fgets(id, sizeof(id), fp) != NULL) {
		...
		if (lg->debug & 1) {
			MT_fprintf(stderr, "#logger_readlogs last logger id written in %s is " LLFMT "\n", filename, lid);
		}
...
{
	if (lg->debug & 1)
		MT_fprintf(stderr, "#logger_commit\n");
...
bm_subcommit(logger *lg, BAT *list_bid, BAT *list_nme, BAT *catalog_bid, BAT *catalog_nme, BAT *catalog_tpe, BAT *catalog_oid, BAT *dcatalog, BAT *extra, int debug)
{
	...
	BATloop(list_bid, p, q) {
		...
		if (list_bid == catalog_bid && BUNfnd(dcatalog, &pos) != BUN_NONE)
			continue;
		if (debug & 1)
			MT_fprintf(stderr, "#commit new %s (%d) %s\n",
				BBPname(col), col,
	...
	if (extra) {
		...
		BATloop(extra, p, q) {
			...
			if (debug & 1)
				MT_fprintf(stderr, "#commit extra %s %s\n",
					name,
					(list_bid == catalog_bid) ? (char *) BUNtvar(iter, p) : "snapshot");
	...
	GDKfree(n);
	if (res != GDK_SUCCEED)
		MT_fprintf(stderr, "!ERROR: bm_subcommit: commit failed\n");
	return res;
}
...
logger_load(int debug, const char *fn, char filename[FILENAME_MAX], logger *lg)
{
	...
	if (catalog_bid == 0) {
		...
		if (debug & 1)
			MT_fprintf(stderr, "#create %s catalog\n", fn);
...
logger_new(int debug, const char *fn, const char *logdir, int version, preversionfix_fptr prefuncp, postversionfix_fptr postfuncp)
{
	...
	if (MT_path_absolute(logdir)) {
		MT_fprintf(stderr, "!ERROR: logger_new: logdir must be relative path\n");
		return NULL;
	}

	lg = GDKmalloc(sizeof(struct logger));
	if (lg == NULL) {
		MT_fprintf(stderr, "!ERROR: logger_new: allocating logger structure failed\n");
		return NULL;
	}
	...
	if (lg->fn == NULL || lg->dir == NULL || lg->buf == NULL) {
		MT_fprintf(stderr, "!ERROR: logger_new: strdup failed\n");
		GDKfree(lg->fn);
		GDKfree(lg->dir);
		GDKfree(lg->buf);
		GDKfree(lg);
	...
	if (lg->debug & 1) {
		MT_fprintf(stderr, "#logger_new dir set to %s\n", lg->dir);
	}
	lg->local_dir = NULL;
...
logger_reload(logger *lg)
{
	...
	if (lg->debug & 1) {
		MT_fprintf(stderr, "#logger_reload %s\n", filename);
	}
...
logger_create(int debug, const char *fn, const char *logdir, int version, preversionfix_fptr prefuncp, postversionfix_fptr postfuncp)
{
	...
	if (lg->debug & 1) {
		MT_fprintf(stdout, "# Finished processing logs %s/%s\n",fn,logdir);
	}
...
logger_destroy(logger *lg)
{
	if (lg->catalog_bid) {
		...
		if (logger_cleanup(lg) != GDK_SUCCEED)
			MT_fprintf(stderr, "#logger_destroy: logger_cleanup failed\n");
...
logger_exit(logger *lg)
{
	...
	if (GDKmove(farmid, lg->dir, LOGFILE, NULL, lg->dir, LOGFILE, "bak") != GDK_SUCCEED) {
		MT_fprintf(stderr, "!ERROR: logger_exit: rename %s to %s.bak in %s failed\n",
	...
	if (len == -1 || len >= FILENAME_MAX) {
		MT_fprintf(stderr, "!ERROR: logger_exit: logger filename path is too large\n");
		return GDK_FAIL;
	}
	if ((fp = GDKfileopen(farmid, NULL, filename, NULL, "w")) != NULL) {
		...
		if (fprintf(fp, "%06d\n\n", lg->version) < 0) {
			(void) fclose(fp);
			MT_fprintf(stderr, "!ERROR: logger_exit: write to %s failed\n",
				filename);
			return GDK_FAIL;
		}
		lg->id ++;
		...
		if (logger_commit(lg) != GDK_SUCCEED) {
			(void) fclose(fp);
			MT_fprintf(stderr, "!ERROR: logger_exit: logger_commit failed\n");
			return GDK_FAIL;
		}
		...
		if (fflush(fp) < 0 ||
			...
			(void) fclose(fp);
			MT_fprintf(stderr, "!ERROR: logger_exit: flush of %s failed\n",
				filename);
			return GDK_FAIL;
		}
		if (fclose(fp) < 0) {
		...
		if (GDKmove(farmid, lg->dir, LOGFILE, "bak", lg->dir, LOGFILE, ext) != GDK_SUCCEED) {
			MT_fprintf(stderr, "!ERROR: logger_exit: rename %s.bak to %s.%s failed\n",
				LOGFILE, LOGFILE, ext);
			return GDK_FAIL;
		}
		...
		MT_fprintf(stderr, "!ERROR: logger_exit: could not create %s\n",
			filename);
...
logger_cleanup(logger *lg)
{
	...
	if (lg->debug & 1) {
		MT_fprintf(stderr, "#logger_cleanup %s\n", buf);
	}
	...
	if ((fp = GDKfileopen(farmid, NULL, buf, NULL, "r")) == NULL) {
		MT_fprintf(stderr, "!ERROR: logger_cleanup: cannot open file %s\n", buf);
		return GDK_FAIL;
	}
	...
	while (lid-- > 0) {
		...
		if (GDKunlink(farmid, lg->dir, LOGFILE, log_id) != GDK_SUCCEED) {
			...
			MT_fprintf(stderr, "#logger_cleanup: failed to remove old WAL %s.%s\n", LOGFILE, buf);
			GDKclrerr();
		}
	}
	fclose(fp);
...
logger_read_last_transaction_id(logger *lg, char *dir, char *logger_file, role_t role)
{
	...
	if (len == -1 || len >= FILENAME_MAX) {
		MT_fprintf(stderr, "!ERROR: logger_read_last_transaction_id: logger filename path is too large\n");
		return -1;
	}
	if ((fp = GDKfileopen(farmid, NULL, filename, NULL, "r")) == NULL) {
		MT_fprintf(stderr, "!ERROR: logger_read_last_transaction_id: unable to open file %s\n", filename);
		return -1;
	}

	if (check_version(lg, fp) != GDK_SUCCEED) {
		MT_fprintf(stderr, "!ERROR: logger_read_last_transaction_id: inconsistent log version for file %s\n", filename);
		fclose(fp);
		return -1;
	}
	...
	while (fgets(id, sizeof(id), fp) != NULL) {
		...
		if (lg->debug & 1) {
			MT_fprintf(stderr, "#logger_read_last_transaction_id last logger id written in %s is " LLFMT "\n", filename, lid);
		}
	}
	fclose(fp);
	return lid;
...
log_bat_persists(logger *lg, BAT *b, const char *name, char tpe, oid id)
{
	...
	if (lg->debug & 1)
		MT_fprintf(stderr, "#persists bat %s (%d) %s\n",
			name, b->batCacheid,
	...
	if (!mnstr_writeInt(lg->log, len) ||
		...
		MT_fprintf(stderr, "!ERROR: log_bat_persists: write failed\n");
		return GDK_FAIL;
	}

	if (lg->debug & 1)
		MT_fprintf(stderr, "#Logged new bat [%s,%s] %s " BUNFMT " (%d)\n",
...
log_bat_transient(logger *lg, const char *name, char tpe, oid id)
{
	...
	if (log_write_format(lg, &l) != GDK_SUCCEED ||
		...
		MT_fprintf(stderr, "!ERROR: log_bat_transient: write failed\n");
		return GDK_FAIL;
	}

	if (lg->debug & 1)
		MT_fprintf(stderr, "#Logged destroyed bat %s\n", NAME(name, tpe, id));
	return GDK_SUCCEED;
}
...
log_delta(logger *lg, BAT *uid, BAT *uval, const char *name, char tpe, oid id)
{
	...
	if (l.nr) {
		...
		if (lg->debug & 1)
			MT_fprintf(stderr, "#Logged %s " LLFMT " inserts\n", name, l.nr);
	}
	if (ok != GDK_SUCCEED)
		MT_fprintf(stderr, "!ERROR: log_delta: write failed\n");
	return ok;
}
...
log_bat(logger *lg, BAT *b, const char *name, char tpe, oid id)
{
	...
	if (ok != GDK_SUCCEED)
		MT_fprintf(stderr, "!ERROR: log_bat: write failed\n");
	return ok;
}
...
log_bat_clear(logger *lg, const char *name, char tpe, oid id)
{
	...
	if (lg->debug & 1)
		MT_fprintf(stderr, "#Logged clear %s\n", NAME(name, tpe, id));
...
log_tstart(logger *lg)
{
	...
	if (lg->debug & 1)
		MT_fprintf(stderr, "#log_tstart %d\n", lg->tid);
...
log_tend(logger *lg)
{
	...
	if (DELTAdirty(lg->snapshots_bid)) {
		...
		if (tids == NULL) {
			MT_fprintf(stderr, "!ERROR: log_tend: bm_tids failed\n");
			return GDK_FAIL;
		}
		cands = BATselect(lg->snapshots_tid, tids, &lg->tid, &lg->tid,
				     true, true, false);
		if (cands == NULL) {
			MT_fprintf(stderr, "!ERROR: log_tend: select failed\n");
			return GDK_FAIL;
		}
		...
		if (bids == NULL) {
			MT_fprintf(stderr, "!ERROR: log_tend: project failed\n");
			return GDK_FAIL;
		}
	...
	if (res != GDK_SUCCEED ||
		...
		MT_fprintf(stderr, "!ERROR: log_tend: write failed\n");
		return GDK_FAIL;
	}
	return GDK_SUCCEED;
}
...
log_sequence_(logger *lg, int seq, lng val, int flush)
{
	...
	if (lg->debug & 1)
		MT_fprintf(stderr, "#log_sequence_ (%d," LLFMT ")\n", seq, val);
	...
	if (log_write_format(lg, &l) != GDK_SUCCEED ||
	    ...
	    pre_allocate(lg) != GDK_SUCCEED) {
		MT_fprintf(stderr, "!ERROR: log_sequence_: write failed\n");
		return GDK_FAIL;
	}
	return GDK_SUCCEED;
}
...
log_sequence(logger *lg, int seq, lng val)
{
	...
	if (lg->debug & 1)
		MT_fprintf(stderr, "#log_sequence (%d," LLFMT ")\n", seq, val);
...
bm_commit(logger *lg)
{
	...
	for (p = b->batInserted; p < BUNlast(b); p++) {
		...
		if (lg->debug & 1)
			MT_fprintf(stderr, "#bm_commit: create %d (%d)\n",
				bid, BBP_lrefs(bid));
	}
