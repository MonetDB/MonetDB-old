...
#ifdef MMAP_DEBUG
	mnstr_printf(GDKerr, "#munmap(%p,%zu) = %d\n", p, len, ret);
#endif
	return ret;
}
	...
	*new_size = (*new_size + GDK_mmap_pagesize - 1) & ~(GDK_mmap_pagesize - 1);
	...
	if (*new_size < old_size) {
		...
		if (path && truncate(path, *new_size) < 0)
			mnstr_printf(GDKerr, "#MT_mremap(%s): truncate failed\n", path);
#ifdef MMAP_DEBUG
		mnstr_printf(GDKerr, "MT_mremap(%s,%p,%zu,%zu) -> shrinking\n", path?path:"NULL", old_address, old_size, *new_size);
#endif
	...
	if (*new_size == old_size) {
		/* do nothing */
#ifdef MMAP_DEBUG
		mnstr_printf(GDKerr, "MT_mremap(%s,%p,%zu,%zu) -> unchanged\n", path?path:"NULL", old_address, old_size, *new_size);
#endif
		return old_address;
	}
	...
	if (!(mode & MMAP_COPY) && path != NULL) {
		...
		if ((fd = open(path, O_RDWR | O_CLOEXEC)) < 0) {
			...
			mnstr_printf(GDKerr, "= %s:%d: MT_mremap(%s,%p,%zu,%zu): open() failed\n", __FILE__, __LINE__, path, old_address, old_size, *new_size);
			return NULL;
		}
		if (GDKextendf(fd, *new_size, path) != GDK_SUCCEED) {
			close(fd);
			mnstr_printf(GDKerr, "= %s:%d: MT_mremap(%s,%p,%zu,%zu): GDKextendf() failed\n", __FILE__, __LINE__, path, old_address, old_size, *new_size);
			return NULL;
		}
#ifdef HAVE_MREMAP
		/* on Linux it's easy */
		...
		if ((fd = open("/dev/zero", O_RDWR | O_CLOEXEC)) < 0) {
			...
			mnstr_printf(GDKerr, "= %s:%d: MT_mremap(%s,%p,%zu,%zu): open('/dev/zero') failed\n", __FILE__, __LINE__, path?path:"NULL", old_address, old_size, *new_size);
			return NULL;
		}
#endif
			...
			if (p == (char *) old_address + old_size) {
				...
				if (p == MAP_FAILED && path != NULL) {
					...
					if (p == NULL){
						...
						mnstr_printf(GDKerr, "= %s:%d: MT_mremap(%s,%p,%zu,%zu): fd < 0\n", __FILE__, __LINE__, path, old_address, old_size, *new_size);
						return NULL;
					}
...
#ifdef MMAP_DEBUG
	mnstr_printf(GDKerr, "MT_mremap(%s,%p,%zu,%zu) -> %p%s\n", path?path:"NULL", old_address, old_size, *new_size, p, path && mode & MMAP_COPY ? " private" : "");
#endif
	if (p == MAP_FAILED)
		mnstr_printf(GDKerr, "= %s:%d: MT_mremap(%s,%p,%zu,%zu): p == MAP_FAILED\n", __FILE__, __LINE__, path?path:"NULL", old_address, old_size, *new_size);
...
MT_mremap(const char *path, int mode, void *old_address, size_t old_size, size_t *new_size)
{
	...
	*new_size = (*new_size + GDK_mmap_pagesize - 1) & ~(GDK_mmap_pagesize - 1);
	...
	if (GDKextend(path, *new_size) != GDK_SUCCEED) {
		mnstr_printf(GDKerr, "= %s:%d: MT_mremap(%s,%p,%zu,%zu): GDKextend() failed\n", __FILE__, __LINE__, path?path:"NULL", old_address, old_size, *new_size);
		return NULL;
	}
...
#ifdef MMAP_DEBUG
	mnstr_printf(GDKerr, "MT_mremap(%s,%p,%zu,%zu) -> %p\n", path?path:"NULL", old_address, old_size, *new_size, p);
#endif
	if (p == NULL)
		mnstr_printf(GDKerr, "= %s:%d: MT_mremap(%s,%p,%zu,%zu): p == NULL\n", __FILE__, __LINE__, path?path:"NULL", old_address, old_size, *new_size);
	return p;
}
...
win_rmdir(const char *pathname)
{
	...
	if (ret < 0 && errno != ENOENT) {
		...
		IODEBUG mnstr_printf(GDKerr, "retry rmdir %s\n", pathname);
...
win_unlink(const char *pathname)
{
	...
	if (ret < 0 && errno != ENOENT) {
		 ...
		 * service which prevents us from doing what we have a
		 * right to do, so try again (once) */
		IODEBUG mnstr_printf(GDKerr, "retry unlink %s\n", pathname);
		MT_sleep_ms(100);	/* wait a little */
		ret = _unlink(pathname);
	}
	return ret;
...
win_rename(const char *old, const char *dst)
{
	...
	if (ret < 0 && errno != ENOENT) {
		 ...
		 * service which prevents us from doing what we have a
		 * right to do, so try again (once) */
		IODEBUG mnstr_printf(GDKerr, "#retry rename %s %s\n", old, dst);
		MT_sleep_ms(100);	/* wait a little */
		ret = rename(old, dst);
	}
	return ret;
