...
GDKcreatedir(const char *dir)
{
	...
	IODEBUG MT_fprintf(stderr, "#GDKcreatedir(%s)\n", dir);
	assert(!GDKinmemory());
		...
		*r = 0;
		if (
			...
			if (errno != EEXIST) {
				...
				IODEBUG MT_fprintf(stderr, "#GDKcreatedir: mkdir(%s) failed\n", path);
				return GDK_FAIL;
			}
			if ((dirp = opendir(path)) == NULL) {
				GDKsyserror("GDKcreatedir: %s not a directory\n", path);
				IODEBUG MT_fprintf(stderr, "#GDKcreatedir: opendir(%s) failed\n", path);
				return GDK_FAIL;
			}
...
GDKremovedir(int farmid, const char *dirname)
{
	...
	IODEBUG MT_fprintf(stderr, "#GDKremovedir(%s)\n", dirnamestr);
...
GDKfileopen(int farmid, const char *dir, const char *name, const char *extension, const char *mode)
{
	...
	if (path != NULL) {
		FILE *f;
		IODEBUG MT_fprintf(stderr, "#GDKfileopen(%s)\n", path);
		f = fopen(path, mode);
		GDKfree(path);
		return f;
	}
...
GDKunlink(int farmid, const char *dir, const char *nme, const char *ext)
{
	if (nme && *nme) {
		...
		if (remove(path) != 0 && errno != ENOENT) {
			...
			IODEBUG MT_fprintf(stderr, "#remove %s = -1\n", path);
			GDKfree(path);
			return GDK_FAIL;
		}
		GDKfree(path);
...
GDKmove(int farmid, const char *dir1, const char *nme1, const char *ext1, const char *dir2, const char *nme2, const char *ext2)
{
	...
	if (path1 && path2) {
		...
		IODEBUG MT_fprintf(stderr, "#move %s %s = %d (%dms)\n", path1, path2, ret, GDKms() - t0);
	} else {
		ret = -1;
	}
	GDKfree(path1);
...
GDKextendf(int fd, size_t size, const char *fn)
{
	...
	IODEBUG MT_fprintf(stderr, "#GDKextend %s %zu -> %zu %dms%s\n",
...
GDKsave(int farmid, const char *nme, const char *ext, void *buf, size_t size, storage_t mode, bool dosync)
{
	...
	IODEBUG MT_fprintf(stderr, "#GDKsave: name=%s, ext=%s, mode %d, dosync=%d\n", nme, ext ? ext : "", (int) mode, dosync);
...
GDKload(int farmid, const char *nme, const char *ext, size_t size, size_t *maxsize, storage_t mode)
{
	...
	IODEBUG {
		MT_fprintf(stderr, "#GDKload: name=%s, ext=%s, mode %d\n", nme, ext ? ext : "", (int) mode);
	}
	if (mode == STORE_MEM) {
		...
		if (fd >= 0) {
			...
			if (ret) {
				...
				for (n_expected = (ssize_t) size; n_expected > 0; n_expected -= n) {
					 ...
					 * not its contents */
					IODEBUG MT_fprintf(stderr, "#read(dst %p, n_expected %zd, fd %d) = %zd\n", (void *)dst, n_expected, fd, n);
#endif
		...
		if (nme != NULL && GDKextend(nme, size) == GDK_SUCCEED) {
			...
			IODEBUG MT_fprintf(stderr, "#mmap(NULL, 0, maxsize %zu, mod %d, path %s, 0) = %p\n", size, mod, nme, (void *)ret);
		}
		GDKfree(path);
	}
	return ret;
...
BATdelete(BAT *b)
{
	...
	if (b->batCopiedtodisk || (b->theap.storage != STORE_MEM)) {
		if (b->ttype != TYPE_void &&
		    ...
		    b->batCopiedtodisk)
			IODEBUG MT_fprintf(stderr, "#BATdelete(%s): bun heap\n", BATgetId(b));
	...
	if (b->tvheap) {
		...
		if (b->batCopiedtodisk || (b->tvheap->storage != STORE_MEM)) {
			if (HEAPdelete(b->tvheap, o, "theap") != GDK_SUCCEED &&
			    b->batCopiedtodisk)
				IODEBUG MT_fprintf(stderr, "#BATdelete(%s): tail heap\n", BATgetId(b));
		} else {
