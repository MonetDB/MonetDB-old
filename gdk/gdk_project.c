...
BATproject(BAT *l, BAT *r)
{
	...
	ALGODEBUG mnstr_printf(GDKerr, "#BATproject(l=" ALGOBATFMT ","
			  "r=" ALGOBATFMT ")\n",
	...
	if (BATtdense(l) && BATcount(l) > 0) {
		...
		ALGODEBUG mnstr_printf(GDKerr, "#BATproject(l=%s,r=%s)=" ALGOOPTBATFMT " (slice)\n",
	...
	if (l->ttype == TYPE_void || BATcount(l) == 0 ||
		...
		ALGODEBUG mnstr_printf(GDKerr, "#BATproject(l=%s,r=%s)=" ALGOOPTBATFMT " (constant)\n",
				  BATgetId(l), BATgetId(r), ALGOOPTBATPAR(bn));
		return bn;
	}
	...
	if (bn == NULL) {
		ALGODEBUG mnstr_printf(GDKerr, "#BATproject(l=%s,r=%s)=0\n",
	...
	ALGODEBUG mnstr_printf(GDKerr, "#BATproject(l=%s,r=%s)=" ALGOBATFMT "%s " LLFMT "us\n",
...
BATprojectchain(BAT **bats)
{
	...
	if (allnil) {
		...
		ALGODEBUG mnstr_printf(GDKerr, "#BATprojectchain with %d BATs, size "BUNFMT", type %s, all nil\n", n, cnt, ATOMname(tpe));
		GDKfree(ba);
	...
	if (i == 1) {
		...
		ALGODEBUG mnstr_printf(GDKerr, "#BATprojectchain with %d BATs, "
	...
	ALGODEBUG mnstr_printf(GDKerr, "#BATprojectchain with %d (%d) BATs, size "BUNFMT", type %s\n", n, i, cnt, ATOMname(tpe));
...
#define TOTPE	TYPE_int
	...
	bn->tseqbase = oid_nil;
	GDKfree(ba);
	ALGODEBUG mnstr_printf(GDKerr, "#BATprojectchain()=" ALGOBATFMT "\n",
			  ALGOBATPAR(bn));
	return bn;
