...
BATappend(BAT *b, BAT *n, BAT *s, bool force)
{
	...
	CHECKDEBUG {
		if (BATttype(b) != BATttype(n) &&
			...
			mnstr_printf(GDKerr,"#Interpreting %s as %s.\n",
		...
		if (b->tsorted || b->trevsorted) {
			...
			for (q = BUNlast(b), p = 1; p < q; p++) {
				...
				if ((*cmpf)(prev, cur) == 0) {
					b->tnokey[0] = p - 1;
					b->tnokey[1] = p;
					ALGODEBUG mnstr_printf(GDKerr, "#BATkeyed: fixed nokey(" BUNFMT "," BUNFMT ") for %s#" BUNFMT " (" LLFMT " usec)\n", p - 1, p, BATgetId(b), BATcount(b), GDKusec() - t0);
					goto doreturn;
				}
				prev = cur;
			}
	...
	if (!b->tsorted && b->tnosorted == 0) {
		...
		case TYPE_int: {
			...
			for (q = BUNlast(b), p = 1; p < q; p++) {
				if (iptr[p - 1] > iptr[p]) {
					b->tnosorted = p;
					ALGODEBUG mnstr_printf(GDKerr, "#BATordered: fixed nosorted(" BUNFMT ") for %s#" BUNFMT " (" LLFMT " usec)\n", p, BATgetId(b), BATcount(b), GDKusec() - t0);
					goto doreturn;
				} else if (!b->trevsorted &&
					   b->tnorevsorted == 0 &&
					   iptr[p - 1] < iptr[p]) {
					b->tnorevsorted = p;
					ALGODEBUG mnstr_printf(GDKerr, "#BATordered: fixed norevsorted(" BUNFMT ") for %s#" BUNFMT "\n", p, BATgetId(b), BATcount(b));
				}
			}
			break;
		}
		 ...
		 * sorted */
		b->tsorted = true;
		ALGODEBUG mnstr_printf(GDKerr, "#BATordered: fixed sorted for %s#" BUNFMT " (" LLFMT " usec)\n", BATgetId(b), BATcount(b), GDKusec() - t0);
		if (!b->trevsorted && b->tnorevsorted == 0) {
			b->trevsorted = true;
			ALGODEBUG mnstr_printf(GDKerr, "#BATordered: fixed revsorted for %s#" BUNFMT "\n", BATgetId(b), BATcount(b));
		}
	}
  doreturn:
	MT_lock_unset(&GDKhashLock(b->batCacheid));
	...
	if (!b->trevsorted && b->tnorevsorted == 0) {
		...
		for (q = BUNlast(b), p = 1; p < q; p++) {
			if (cmpf(BUNtail(bi, p - 1), BUNtail(bi, p)) < 0) {
				b->tnorevsorted = p;
				ALGODEBUG mnstr_printf(GDKerr, "#BATordered_rev: fixed norevsorted(" BUNFMT ") for %s#" BUNFMT " (" LLFMT " usec)\n", p, BATgetId(b), BATcount(b), GDKusec() - t0);
				goto doreturn;
			}
		}
		b->trevsorted = true;
		ALGODEBUG mnstr_printf(GDKerr, "#BATordered_rev: fixed revsorted for %s#" BUNFMT " (" LLFMT " usec)\n", BATgetId(b), BATcount(b), GDKusec() - t0);
	}
  doreturn:
	MT_lock_unset(&GDKhashLock(b->batCacheid));
	return b->trevsorted;
	...
	BAT *b, BAT *o, BAT *g, bool reverse, bool nilslast, bool stable)
{
	...
	if (BATcount(b) <= 1 ||
		...
		ALGODEBUG mnstr_printf(GDKerr, "#BATsort(b=" ALGOBATFMT ",o="
	...
	if (g) {
		if (g->tkey || g->ttype == TYPE_void) {
			...
			ALGODEBUG mnstr_printf(GDKerr, "#BATsort(b=" ALGOBATFMT
	...
	ALGODEBUG mnstr_printf(GDKerr, "#BATsort(b=" ALGOBATFMT ",o=" ALGOOPTBATFMT
...
BATconstant(oid hseq, int tailtype, const void *v, BUN n, int role)
{
	...
	ALGODEBUG mnstr_printf(GDKerr, "#BATconstant()=" ALGOBATFMT "\n", ALGOBATPAR(bn));
	return bn;
