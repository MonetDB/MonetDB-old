...
#define MAYBEEXTEND_PROGRESS(CNT, PROGRESS)				\
	do {								\
		...
		if (BATcount(r1) + N > BATcapacity(r1)) {		\
			...
			/* far) */					\
			BUN newcap = (BUN) ((double) lcnt / (lcnt - (PROGRESS)) * (BATcount(r1) + N) * 1.5); \
...
#define MAYBEEXTEND(CNT)	MAYBEEXTEND_PROGRESS(CNT, lcand ? (BUN) (lcandend - lcand) : (lend - lstart))
	...
	bool nil_on_miss, bool only_misses, const char *func, lng t0)
{
	...
	if (r2p) {
		if ((r2 = BATconstant(0, TYPE_void, &oid_nil, cnt, TRANSIENT)) == NULL) {
			BBPreclaim(r1);
			return GDK_FAIL;
		}
		*r2p = r2;
	   ...
	   bool nil_matches, lng t0, bool swapped)
{
	...
	ALGODEBUG mnstr_printf(GDKerr, "#selectjoin(l=" ALGOBATFMT ","
	...
	*r1p = r1;
	*r2p = r2;
	ALGODEBUG mnstr_printf(GDKerr, "#selectjoin(l=%s,r=%s)=(" ALGOBATFMT "," ALGOOPTBATFMT ") " LLFMT "us\n",
	       ...
	       bool nil_on_miss, bool only_misses, lng t0, bool swapped)
{
	...
	ALGODEBUG mnstr_printf(GDKerr, "#mergejoin_void(l=" ALGOBATFMT ","
			  "r=" ALGOBATFMT ",sl=" ALGOOPTBATFMT ","
			  "sr=" ALGOOPTBATFMT ","
  ...
  doreturn:
	...
	if (r2->tkey && r2->tsorted)
		virtualize(r2);
  doreturn2:
	ALGODEBUG mnstr_printf(GDKerr, "#mergejoin_void(l=%s,r=%s)=(" ALGOBATFMT "," ALGOOPTBATFMT ") " LLFMT "us\n",
			  BATgetId(l), BATgetId(r),
			  ALGOBATPAR(r1), ALGOOPTBATPAR(r2),
			  GDKusec() - t0);
	return GDK_SUCCEED;
	      ...
	      bool nil_matches, BUN estimate, lng t0, bool swapped)
{
	...
	ALGODEBUG mnstr_printf(GDKerr, "#mergejoin_int(l=" ALGOBATFMT ","
			  "r=" ALGOBATFMT ")%s\n",
	...
	ALGODEBUG mnstr_printf(GDKerr, "#mergejoin_int(l=%s,r=%s)=(" ALGOBATFMT "," ALGOBATFMT ") " LLFMT "us\n",
			  BATgetId(l), BATgetId(r),
	      ...
	      bool nil_matches, BUN estimate, lng t0, bool swapped)
{
	...
	ALGODEBUG mnstr_printf(GDKerr, "#mergejoin_lng(l=" ALGOBATFMT ","
			  "r=" ALGOBATFMT ")%s\n",
			  ALGOBATPAR(l), ALGOBATPAR(r),
			  swapped ? " swapped" : "");
	...
	ALGODEBUG mnstr_printf(GDKerr, "#mergejoin_lng(l=%s,r=%s)=(" ALGOBATFMT "," ALGOBATFMT ") " LLFMT "us\n",
			  BATgetId(l), BATgetId(r),
			  ALGOBATPAR(r1), ALGOBATPAR(r2),
			  GDKusec() - t0);
	return GDK_SUCCEED;
	  ...
	  BUN estimate, lng t0, bool swapped)
{
	...
	ALGODEBUG mnstr_printf(GDKerr, "#mergejoin(l=" ALGOBATFMT ","
			  "r=" ALGOBATFMT ",sl=" ALGOOPTBATFMT ","
	...
	ALGODEBUG mnstr_printf(GDKerr, "#mergejoin(l=%s,r=%s)=(" ALGOBATFMT "," ALGOOPTBATFMT ") " LLFMT "us\n",
			  BATgetId(l), BATgetId(r),
			  ALGOBATPAR(r1), ALGOOPTBATPAR(r2),
			  GDKusec() - t0);
	return GDK_SUCCEED;
	 ...
	 BUN estimate, lng t0, bool swapped, bool phash, const char *reason)
{
	...
	(void) rcandend;
	ALGODEBUG mnstr_printf(GDKerr, "#hashjoin(l=" ALGOBATFMT ","
			  "r=" ALGOBATFMT ",sl=" ALGOOPTBATFMT ","
			  "sr=" ALGOOPTBATFMT ",nil_matches=%d,"
	...
	if (phash) {
		...
		assert(sr == NULL);
		ALGODEBUG mnstr_printf(GDKerr, "#hashjoin(%s): using "
	...
	if (sr) {
		if (BATtdense(sr) &&
			...
			char ext[32];
			assert(!phash);
			ALGODEBUG mnstr_printf(GDKerr, "#hashjoin(%s): creating "
	...
	ALGODEBUG mnstr_printf(GDKerr, "#hashjoin(l=%s,r=%s)=(" ALGOBATFMT ","
...
thetajoin(BAT **r1p, BAT **r2p, BAT *l, BAT *r, BAT *sl, BAT *sr, int opcode, BUN estimate, lng t0)
{
	...
	ALGODEBUG mnstr_printf(GDKerr, "#thetajoin(l=" ALGOBATFMT ","
			  "r=" ALGOBATFMT ",sl=" ALGOOPTBATFMT ","
	...
	ALGODEBUG mnstr_printf(GDKerr, "#thetajoin(l=%s,r=%s)=(" ALGOBATFMT "," ALGOBATFMT ") " LLFMT "us\n",
			  BATgetId(l), BATgetId(r),
			  ALGOBATPAR(r1), ALGOBATPAR(r2),
			  GDKusec() - t0);
	return GDK_SUCCEED;
	 ...
	 lng t0)
{
	...
	ALGODEBUG mnstr_printf(GDKerr, "#BATbandjoin(l=%s,r=%s)=(" ALGOBATFMT "," ALGOBATFMT ") " LLFMT "us\n",
			  BATgetId(l), BATgetId(r),
			  ALGOBATPAR(r1), ALGOBATPAR(r2),
			  GDKusec() - t0);
	return GDK_SUCCEED;
	  ...
	  BUN rstart, BUN rend, lng t0)
{
	...
	ALGODEBUG mnstr_printf(GDKerr, "#fetchjoin(l=" ALGOBATFMT ","
			  "r=" ALGOBATFMT ")\n",
			  ALGOBATPAR(l), ALGOBATPAR(r));
	...
	*r1p = r1;
	...
	ALGODEBUG mnstr_printf(GDKerr, "#fetchjoin(l=%s,r=%s)=(" ALGOBATFMT "," ALGOOPTBATFMT ") " LLFMT "us\n",
			  BATgetId(l), BATgetId(r),
			  ALGOBATPAR(r1), ALGOOPTBATPAR(r2),
			  GDKusec() - t0);
	return GDK_SUCCEED;
	 ...
	 BUN estimate, const char *func, lng t0)
{
	...
	*r1p = NULL;
	...
	if (lcnt == 0 || (!only_misses && !nil_on_miss && rcnt == 0)) {
		ALGODEBUG mnstr_printf(GDKerr, "#%s(l=" ALGOBATFMT ","
				  "r=" ALGOBATFMT ",sl=" ALGOOPTBATFMT ","
				  "sr=" ALGOOPTBATFMT ",nil_matches=%d,"
...
BATjoin(BAT **r1p, BAT **r2p, BAT *l, BAT *r, BAT *sl, BAT *sr, bool nil_matches, BUN estimate)
{
	...
	*r2p = NULL;
	...
	if (lcnt == 0 || rcnt == 0) {
		ALGODEBUG mnstr_printf(GDKerr, "#BATjoin(l=" ALGOBATFMT ","
				  "r=" ALGOBATFMT ",sl=" ALGOOPTBATFMT ","
				  "sr=" ALGOOPTBATFMT ",nil_matches=%d)\n",
				  ALGOBATPAR(l), ALGOBATPAR(r),
				  ALGOOPTBATPAR(sl), ALGOOPTBATPAR(sr),
	       ...
	       const void *c1, const void *c2, bool li, bool hi, BUN estimate)
{
	...
	ALGODEBUG mnstr_printf(GDKerr, "#BATbandjoin("
