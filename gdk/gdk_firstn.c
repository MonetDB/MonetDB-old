...
BATfirstn_unique(BAT *b, BAT *s, BUN n, bool asc, bool nilslast, oid *lastp)
{
	...
	if (cand) {
		...
		if (n >= cnt) {
			...
			if (lastp)
				*lastp = 0;
			return BATfixcand(BATslice(s, start, end));
		}
	} else {
		cnt = end - start;
		if (n >= cnt) {
	...
	if (BATtvoid(b)) {
		...
		if (asc || is_oid_nil(b->tseqbase)) {
			...
			if (cand) {
				if (lastp)
					*lastp = cand[n - 1];
				return BATfixcand(BATslice(s, start, start + n));
			}
			if (lastp)
				*lastp = is_oid_nil(b->tseqbase)
					? oid_nil : b->hseqbase + n - 1;
			return BATfixcand(BATdense(0, b->hseqbase, n));
		}
		...
		if (cand) {
			if (lastp)
				*lastp = cand[cnt - n];
			return BATfixcand(BATslice(s, end - n, end));
		}
		if (lastp)
			*lastp = b->hseqbase + end - n;
		return BATfixcand(BATdense(0, b->hseqbase + end - n, n));
	}
	...
	if (BATordered(b) | BATordered_rev(b)) {
			...
			if (nilslast == asc && !b->tnonil) {
				...
				if (b->tsorted) {
					...
					if (asc) { /* i.e. nilslast */
						...
						if (lastp)
							*lastp = cand[pos + n - 1];
						return BATfixcand(BATslice(s, start + pos, start + pos + n));
					...
					if (asc) { /* i.e. nilslast */
						...
						if (lastp)
							*lastp = cand[pos - n];
						return BATfixcand(BATslice(s, start + pos - n, start + pos));
					} else { /* i.e. !asc, !nilslast */
						/* prefer nil and largest */
						if (cnt - pos < n) {
							if (lastp)
		...
		if (nilslast == asc && !b->tnonil) {
			...
			if (b->tsorted) {
				...
				if (asc) { /* i.e. nilslast */
					...
					return BATfixcand(BATdense(0, b->hseqbase + pos, n));
				} else { /* i.e. !asc, !nilslast */
					/* prefer nil and largest */
					if (pos < n) {
						if (lastp)
				...
				if (asc) { /* i.e. nilslast */
					...
					return BATfixcand(BATdense(0, b->hseqbase + pos - n, n));
				} else { /* i.e. !asc, !nilslast */
					/* prefer nil and largest */
					if (end - pos < n) {
						if (lastp)
		...
		if (asc ? b->tsorted : b->trevsorted) {
			...
			bn = BATdense(0, b->hseqbase + start, n);
		} else {
			/* last n entries from b */
			if (lastp)
				*lastp = b->hseqbase + start + cnt - n;
			bn = BATdense(0, b->hseqbase + start + cnt - n, n);
		}
		return BATfixcand(bn);
	}
