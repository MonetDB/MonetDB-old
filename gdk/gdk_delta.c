...
BATcommit(BAT *b)
{
	if (b == NULL)
		return;
	DELTADEBUG MT_fprintf(stderr, "#BATcommit1 %s free %zu ins " BUNFMT " base %p\n",
			   BATgetId(b),
			   b->theap.free,
			   b->batInserted,
			   b->theap.base);
	...
	DELTADEBUG MT_fprintf(stderr, "#BATcommit2 %s free %zu ins " BUNFMT " base %p\n",
			   BATgetId(b),
			   b->theap.free,
			   b->batInserted,
			   b->theap.base);
...
BATundo(BAT *b)
{
	...
	if (b == NULL)
		return;
	DELTADEBUG MT_fprintf(stderr, "#BATundo %s \n", BATgetId(b));
