...
virtualize(BAT *bn)
{
	...
	if (bn && bn->ttype == TYPE_oid &&
		...
		ALGODEBUG MT_fprintf(stderr, "#virtualize(bn=" ALGOBATFMT ",seq="OIDFMT")\n",
				  ALGOBATPAR(bn),
...
hashselect(BAT *b, BAT *s, BAT *bn, const void *tl, BUN maximum, bool phash)
{
	...
	if (phash) {
		...
		ALGODEBUG
			MT_fprintf(stderr, "#hashselect(" ALGOBATFMT "): "
	     ...
	     bool li, bool hi, bool anti)
{
	...
	if (BATordered(b) || BATordered_rev(b) || use_orderidx) {
		...
		bn = virtualize(bn);
		ALGODEBUG MT_fprintf(stderr, "#BATselect(b=%s)=" ALGOOPTBATFMT
				  " (" LLFMT " usec)\n",
...
rangejoin(BAT *r1, BAT *r2, BAT *l, BAT *rl, BAT *rh, BAT *sl, BAT *sr, bool li, bool hi, BUN maxsize)
{
	...
	ALGODEBUG MT_fprintf(stderr, "#rangejoin(l=" ALGOBATFMT ","
	...
	ALGODEBUG MT_fprintf(stderr, "#rangejoin(l=%s,rl=%s,rh=%s)="
