...
virtualize(BAT *bn)
{
	...
	if (bn && bn->ttype == TYPE_oid &&
		...
		ALGODEBUG MT_fprintf(stderr, "#%s: %s(bn=" ALGOBATFMT ",seq="OIDFMT")\n",
	...
	*algo = "hashselect";
	if (phash) {
		...
		*algo = "hashselect on parent";
		ALGODEBUG MT_fprintf(stderr, "#%s: %s(" ALGOBATFMT "): "
	     ...
	     bool li, bool hi, bool anti)
{
	...
	if (canditer_init(&ci, b, s) == 0) {
		...
		bn = BATdense(0, 0, 0);
		ALGODEBUG MT_fprintf(stderr, "#%s: %s(b=" ALGOBATFMT
	...
	if (anti) {
		if (lval != hval) {
			...
			if (!(li && hi)) {
				...
				lval = false;
				hval = false;
				ALGODEBUG MT_fprintf(stderr, "#%s: %s(b="
						  ALGOBATFMT ",s="
	...
	if (!anti &&
		...
		ALGODEBUG if (view) MT_fprintf(stderr, "#%s: %s: switch from " ALGOBATFMT " to " ALGOBATFMT " " OIDFMT "-" OIDFMT " hseq " LLFMT "\n", MT_thread_getname(), __func__, ALGOBATPAR(view), ALGOBATPAR(b), vwl, vwh, vwo);
	}
	...
	if (b->tsorted || b->trevsorted || use_orderidx) {
		...
		bn = virtualize(bn);
		ALGODEBUG MT_fprintf(stderr, "#%s: %s(b=" ALGOBATFMT ",anti=%s)="
	...
	bn = virtualize(bn);
	ALGODEBUG MT_fprintf(stderr, "#%s: %s(b=" ALGOBATFMT ",s=" ALGOOPTBATFMT",anti=%s)=" ALGOOPTBATFMT
			  " %s (" LLFMT " usec)\n",
			  MT_thread_getname(), __func__,
			  ALGOBATPAR(b), ALGOOPTBATPAR(s),
			  anti ? "true" : "false",
...
rangejoin(BAT *r1, BAT *r2, BAT *l, BAT *rl, BAT *rh, BAT *sl, BAT *sr, bool li, bool hi, BUN maxsize)
{
	...
	ALGODEBUG MT_fprintf(stderr, "#%s: %s(l=" ALGOBATFMT ","
	...
	ALGODEBUG MT_fprintf(stderr, "#%s: %s(l=%s,rl=%s,rh=%s)="
