...
static char init_docstring[] =
    "monetdb.init(directory) => Initialize the SQL client with the given database directory.";
static char sql_docstring[] =
    "monetdb.sql(query) => Execute a SQL query on the database. Returns the result as a dictionary of Numpy Arrays.";
static char create_docstring[] =
    "monetdb.create(tablename, dictionary), monetdb.create(tablename, column_names, values) => Create a SQL table from the given Python objects, objects must either be a (column name, value) dictionary or a list of column names and a list of values";
static char insert_docstring[] =
    "monetdb.insert(tablename, dictionary), monetdb.insert(tablename, column_names, values) => Insert a set of values into a SQL table";
