...
char* monetdb_get_columns(void* conn, const char* schema_name, const char *table_name, int *column_count, char ***column_names, int **column_types, void*** sql_subtypes) {
	mvc *m;
	sql_schema *s;
	sql_table *t;
	...
	*sql_subtypes = GDKzalloc(sizeof(sql_subtype*) * columns);
	...
	for (n = t->columns.set->h; n; n = n->next) {
		...
		(*((sql_subtype***)sql_subtypes))[c->colnr] = &c->type;
	}
...
char* monetdb_create_table(void *conn, const char *schema, const char *table_name, append_data *ad, int ncols) {
	...
	msg = monetdb_query(c, query, &res);
	if (msg != MAL_SUCCEED) {
		return msg;
	}

	return monetdb_append(c, schema, table_name, ad, ncols);
}
